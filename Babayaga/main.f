...
*********************************************
     ...
     > bornmenovp
      common/reducedtoborn/p1b,p2b,iiiref
...
c      bmin = sqrt(1.d0 - ame**2/emin/emin)
c      tmin = 2.d0*E1**2*(1.d0-b1**2*cmin)
c      tmin = 2.d0*ame*ame - tmin
c      tmax = 2.d0*E1*emin*(1.d0-b1*bmin*cmax)
c      tmax = 2.d0*ame*ame - tmax
...
*******************************************
         ...
         if (fs.eq.'ee'.or.fs.eq.'mm') then
c            call phasespaceNEW(pbeam1p,pbeam2p,p1,p2,qph,ng,am1,am2,
c     .           esoft,cth,w,phsp,ie)
ccc            call phasespaceTMP(pbeam1p,pbeam2p,p1,p2,qph,ng,am1,am2,
...
ccc additive form  sdif = sdif / prod/(1.d0+deltasv) * (1.d0+deltasv+deltah)
         ...
         if (isnan(emtx).or.isnan(sv)) then
...
c               if (store.eq.'yes') call eventstorage(p1,p2,qph)
     ...
     .              call eventstorage_unweighted(p1cut,p2cut,qphcut)
            endif
...
c            call system('cat fifoat2 > /dev/null')
c               call system('echo 1 > fifoat2')            
