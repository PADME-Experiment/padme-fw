...
! written by CMCC, last modified 5/3/2006
     ...
     > bornmenovp 
      ...
      common/for_debug/ppp(0:3),denphsp,dir3(0:3),dir4(0:3),aabr,aabq,
     > aaonm
...
*
         ...
         if(fs.eq.'mm') then
c            call alpha_me('R',1,pbeam1,pbeam2,p1,p2,
c     >                     qph,ialpha,bornme)
...
c            call recola_ampl(ng,pin1,pin2,p1,p2,qph(1,:),brcl,olrcl)
c            brcl = 4.d0*brcl
c            print*,summt2/brcl
c         summt2 = brcl
...
***
         ...
         aaa = prodapx * prodomega

c         if (ng.ge.15) then
c            print*,'>>',prodapx,eikfull,prodomega,xjac,prod,aaa
c         endif         
c         summt2 = bornme*prodapx*prodomega*xjac*eikfull
         summt2 = bornme*aaa*xjac*eikfull
...
*** for recola test ee -> 4-5g 
c         if (ng.ge.2) p6 = qph(2,:)
c         if (ng.eq.3) p7 = qph(3,:)
c         call recola_ampl(ng,pin1,pin2,p1,p2,qph(1,:),brcl,olrcl)
c         brcl = 4.d0*(ng+1)*(ng+2)*brcl
c         print*,summt2/brcl
c         summt2 = brcl
...
c         summt2 = bornme * prodapx * eikfull * prodomega * xjac
         summt2 = bornme * aaa * eikfull * xjac
...
c         S = dot(p1+p2,p1+p2)
c         beta = p1(3)/p1(0)
c         c = p4(3)/sqrt(tridot(p4,p4))
c         c2 = c*c
c         ALPHA2=ALPHA*ALPHA
c         BORN1=2.D0*PI*ALPHA2/S*BETA*BETA/
c     >        (1.D0+BETA*C)/(1.D0-BETA*C)*(1.D0+C2)
c         phsp = 3.1662869888230555d-3
c         print*,born1/smt/phsp*(8.d0 * (sqrt(s)/2.d0)**2)/(2.d0*pi/4.d0)        
         return
      endif
...
cc         smtz = 0.d0
****         
c         call recola_ampl(0,p1,p2,p3,p4,p3,bornrcl,allun)
c         bornrcl = 4.d0*bornrcl
         
c         print*,(smt+smtz)/bornrcl
...
! written by CMCC, last modified 13/10/2005
      ...
      dimension qph(40,0:3),qt(0:3),p3t(0:3),p4t(0:3)
...
! ok a 1 GeV...         
c         call recola_ampl(1,p1,p2,p3,p4,q,realrcl,allun)
c         realrcl = realrcl*4.d0
c         print*,realrcl/bkme
         
         
      elseif (fs.eq.'mm') then
         call formmemm(p1,p2,p3,p4,q,bkme)
      elseif (fs.eq.'gg') then
c         ppdk1 = dot(p2,p3)  ! old
c         pmdk1 = dot(p1,p3)  ! old
c         ppdk2 = dot(p2,p4)  ! old
c         pmdk2 = dot(p1,p4)  ! old
c         ppdk3 = dot(p2,q)  ! old
c         pmdk3 = dot(p1,q)  ! old
c         ppdpm = dot(p1,p2)  ! old
c         call EE3G(PPDK1,PMDK1,PPDK2,PMDK2,PPDK3,PMDK3,PPDPM,  ! old
c     .        bkme)             ! old
c         bkmeold = bkme
c         return
c not needed if using set_momenta_correction_rcl(.true.)
c         call martella(p1,p2,p3,p4,q,p3t,p4t,qt)
c         print*,''
c         print*,p3+p4+q
c         print*,p3t+p4t+qt
c         print*,dot(p3,p3),dot(p4,p4),dot(q,q)
c         print*,dot(p3t,p3t),dot(p4t,p4t),dot(qt,qt)
         
*     * CI SONO WARNING CON LA CONSERVAZIONE!!!
*     * mettendo a .true. set_momenta_correction_rcl se ne vanno
c         qt(0) = sqrt(tridot(q,q))
c         p3t(0) = sqrt(tridot(p3,p3))
c         p4t(0) = sqrt(tridot(p4,p4))
c         qt(1:3)  = q(1:3)
c         p3t(1:3) = p3(1:3)
c         p4t(1:3) = p4(1:3)         
c         call recola_ampl(1,p1,p2,p3t,p4t,qt,realrcl,allun)
c         realrcl = realrcl*24.d0

! ok the following 3 lines         
c         call recola_ampl(1,p1,p2,p3,p4,q,realrcl,allun)
c         realrcl = realrcl*24.d0  ! 24 = 4.d0*(ng+1)*(ng+2) ! which works for any ng!
c         bkme = realrcl
! using exact ME calculated from scratch         
         call ee3gexact(p1,p2,p3,p4,q,bkme)

c         print*,bkme/realrcl-1.d0,bkmeold/realrcl-1.d0
         
c         delta = realrcl/bkme-1.d0
c         if (abs(delta).gt.5d-4) then
c            print*,''
c            print*,delta,realrcl,bkme
c            print*,p3
c            print*,p4
c            print*,q            
c         endif
         
c         if (abs(bkme/realrcl-1.d0).gt.0.1d0) then
c            print*,bkme/realrcl-1.d0,ampl/realrcl-1.d0
c         endif
      endif
      return
      end
*****************
      subroutine ee3gexact(p1,p2,k1,k2,k3,ampl)
c      implicit none
      implicit double precision (Z)
      double precision A11,A12,A13,A14,A15,A16
      double precision     A22,A23,A24,A25,A26
      double precision         A33,A34,A35,A36
      double precision             A44,A45,A46
      double precision                 A55,A56
      double precision                     A66
      
      double precision p1(0:3),p2(0:3)
      double precision k1(0:3),k2(0:3),k3(0:3)
      double precision k1o(0:3),k2o(0:3),k3o(0:3)
      double precision ampl,ampl2
      double precision p1p2,p1k1,p1k2,p1k3
      double precision      p2k1,p2k2,p2k3
      double precision           k1k2,k1k3
      double precision                k2k3
      double precision e2,e4,e6,me2,me4,me6,dot
      external dot
      integer ifirst
      common/firstee3gexact/e2,e4,e6,me2,me4,me6,ifirst
      data ifirst /0/
