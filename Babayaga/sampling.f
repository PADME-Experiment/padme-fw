...
*****
      ...
      do k = 1,n
         call getrnd(csi,1)
         ep(k) = int(ncharged*csi(1) + 1.d0)
         if (ep(k).gt.ncharged) ep(k) = ncharged ! to avoid round-offs...
         if (ep(k).eq.0) ep(k) = 1 ! to avoid round-offs...
      enddo
      return
      end 
*****
...
c      tmi = acos(cmax)
c      tma = acos(cmin)
c      tmih = tmi - 15.d0/180.d0*3.14159265358979d0
c      tmah = tma + 15.d0/180.d0*3.14159265358979d0
c      cmaxh = cos(tmih)
c      cminh = cos(tmah)
      ...
      if (ifirst.eq.0.or.ifrstenbld.eq.0) then
         anorm = -31.d0/3.d0 - 16.d0/(cmaxh - 1.d0) + 9.d0*cmaxh
     .        + cmaxh**2 + cmaxh**3/3.d0 + 16.d0*log(1.d0 - cmaxh)

         anorm = anorm
     .        +31.d0/3.d0 + 16.d0/(cminh - 1.d0) - 9.d0*cminh
     .        - cminh**2 - cminh**3/3.d0 - 16.d0*log(1.d0 - cminh)
         anorm = anorm/2.d0
         fmax = fcos(cmaxh)
         ifirst = 1
      endif
...
c         eb = ecms * 0.5d0
c         xx  = -ame*ame/eb/eb
c         ombet = - 0.5d0*xx*(1.d0 - xx * 0.250d0 + xx*xx * 0.125d0
c     .              - 0.078125d0 * xx*xx*xx)   
c         umb = 1.d0-beta
         
c         print*,ombet-umb,xx*xx * 0.125d0,0.078125d0 * xx*xx*xx
