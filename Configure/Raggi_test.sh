...
if [ ${PROBLEMS} -eq 0 ]; then
    ...
    if [ ! -f /cvmfs/sft.cern.ch/lcg/releases/LCG_94/gcc/8.2.0/x86_64-centos7/setup.sh ]; then
	echo "ERROR - File /cvmfs/sft.cern.ch/lcg/releases/LCG_94/gcc/8.2.0/x86_64-centos7/setup.sh not found"
	PROBLEMS=1
    fi

    # Look for ROOT-5.34.26 configuration script
    if [ ! -f /cvmfs/sft.cern.ch/lcg/releases/LCG_98/ROOT/v6.22.00/x86_64-centos7-gcc8-opt/bin/thisroot.sh ]; then
	echo "ERROR - File /cvmfs/sft.cern.ch/lcg/releases/LCG_96/ROOT/6.18.00/x86_64-centos7-gcc8-opt/bin/thisroot.sh not found"
	PROBLEMS=1
    fi

    # Look for general PADME configuration script in the same dir as this script
    if [ ! -f ${THIS_DIR}/padme_init.sh ]; then
	echo "ERROR - File ${THIS_DIR}/Raggi_test.sh not found."
	PROBLEMS=1
    fi
...
if [ ${PROBLEMS} -eq 0 ]; then
    . /cvmfs/sft.cern.ch/lcg/releases/LCG_94/gcc/8.2.0/x86_64-centos7/setup.sh
    #. /cvmfs/sft.cern.ch/lcg/releases/LCG_96/ROOT/6.18.00/x86_64-centos7-gcc8-opt/bin/thisroot.sh
    . /cvmfs/sft.cern.ch/lcg/releases/LCG_98/ROOT/v6.22.00/x86_64-centos7-gcc8-opt/bin/thisroot.sh
