...
double fitfun(double *x, double *p) {
    double fun = p[0]/TMath::Sqrt(2*TMath::Pi())/p[2] * exp(-0.5 * pow((x[0] - p[1])/p[2], 2)) +
                 p[3]/TMath::Sqrt(2*TMath::Pi())/p[5] * exp(-0.5 * pow((x[0] - p[4])/p[5], 2)) +
                 p[6]/TMath::Sqrt(2*TMath::Pi())/p[8] * exp(-0.5 * pow((x[0] - p[7])/p[8], 2)) + 
                 p[9]/TMath::Sqrt(2*TMath::Pi())/p[11] * exp(-0.5 * pow((x[0] - p[10])/p[11], 2)) + 
                 p[12]/TMath::Sqrt(2*TMath::Pi())/p[14] * exp(-0.5 * pow((x[0] - p[13])/p[14], 2)) + 
                 p[15]/TMath::Sqrt(2*TMath::Pi())/p[17] * exp(-0.5 * pow((x[0] - p[16])/p[17], 2));
    return fun;
}
...
void processrun(int nrun, double voltage, double energy, int npeaks, double xmax, double fitmax, std::vector<double> vecA, std::vector<double> vecQ, std::vector<double> vecSQ) {
    ...
    // Define histogram
    TH1F* hQ = new TH1F(Form("hQ_run%i_%iV", nrun, (int)voltage),"; Q [pC]; Number of events",Nbins,xmin,xmax);
    ...
    TFile *newfile = new TFile(Form("/home/mancinima/LeadGlassCalib2024/KKAnalysis/output/HVFits/%s_%iMeV.root",infileName.Data(), (int)energy), "RECREATE");
    ...
    std::ofstream HVScan(Form("/home/mancinima/LeadGlassCalib2024/KKAnalysis/output/HVFits/HVScan_%iMeV.txt", (int)energy), std::ios::app);
    ...
    for (int i = 0; i < npeaks; i++) {
        ...
        vecSigmaQrelErr.push_back((sigmaQ/meanQ)*TMath::Sqrt(pow((sigmaQerr/sigmaQ),2)+pow((meanQerr/meanQ),2)));
    ...
    graphQ->SetName(Form("graphQ_run%i_%iV", nrun, (int)voltage));
    ...
    graphSigmaQ->SetName(Form("graphSigmaQ_run%i_%iV", nrun, (int)voltage));
...
int fitHVScan() {
    ...
    if (Energy == 285) {
        ...
        // vecsigmaQ = {0.2, 0.22, 0.25, 0.25, 0.4, 0.5};
        // vecA = {200, 250, 250, 200, 100, 50};
        // processrun(299, HV, Energy, Np, 10., 6.2, vecA, vecmeanQ, vecsigmaQ);
    }
