...
void plotChargeHV() {
    // Choose ENERGY !!!
    double Energy = 285.; // 450 // 285 // 204.6 MeV

    TString infileName = Form("HVScan_fitKK_%iMeV.txt",(int)Energy); // HVScan450 // HVScan285 // HVScan204
    double HVmin, HVmax;
    ...
    // Open file
    std::ifstream infile(Form("/home/mancinima/LeadGlassCalib2024/KKAnalysis/output/HVFits/%s",infileName.Data()));
    if (!infile.is_open()) {
    ...
    while (infile >> HV >> QE >> QEerr) {
        std::cout << "HV = " << HV << " - QE = " << QE << " - QEerr = " << QEerr << std::endl;
        vecHV.push_back(HV);
        vecLogHV.push_back(log(HV));
        vecQE.push_back(QE);
    ...
    gQE->SetName(Form("QE_%i", (int)Energy));
    gQE->SetTitle(";HV [V]; Q/E [pC/MeV]");
    ...
    gLogQE->SetName(Form("LogQE_%i", (int)Energy));
    gLogQE->SetTitle(";HV [V]; ln(Q/E) [pC/MeV]");
    ...
    TF1 *logfitfun = new TF1("logfitfun", "[0] + [1]*(x)", log(xmin+25), log(xmax), 2);
    // logfitfun->SetParameter(0, 1);
    // logfitfun->SetParameter(1, 8.);
    // gLogQE->Fit(logfitfun, "R");
    ...
    gLogQELogHV->SetName(Form("LogQELogHV%i", (int)Energy));
    gLogQELogHV->SetTitle(";ln(V) [V]; ln(Q/E) [pC/MeV]");
    ...
    std::cout << "Q1e(900) = " << QE900*Energy << " pC" << std::endl;

    // Create a .root file
    TFile *outfile = new TFile(Form("/home/mancinima/LeadGlassCalib2024/KKAnalysis/output/QEHV_%iMeV.root", (int)Energy), "RECREATE");
