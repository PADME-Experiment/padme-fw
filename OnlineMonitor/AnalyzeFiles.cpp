...
int main(int argc, char* argv[])
{
  ...
  TString outputFileName = "AnalyzeFiles.txt";
  ...
  while ((c = getopt (argc, argv, "i:l:o:n:v:h")) != -1) {
	...
	if (nevents) {
      ...
      case 'v':
        ...
        break;
      case 'h':
        fprintf(stdout,"\nAnalyzeFiles [-i input root file] [-l list of input files] [-v verbosity] [-h]\n\n");
        fprintf(stdout,"  -i: define an input file in root format\n");
        fprintf(stdout,"  -l: define a list of input files\n");
        fprintf(stdout,"  -o: define name of PadmeMonitor-format output file\n");
  ...
  printf("=== AnalyzeFiles starting on %s\n",cfg->FormatTime(now.tv_sec));
  ...
  printf("=== AnalyzeFiles ending on %s\n",cfg->FormatTime(now.tv_sec));
