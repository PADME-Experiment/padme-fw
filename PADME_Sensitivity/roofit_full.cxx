...
void debug_toy(const RooDataSet *toy_experiment) {
  ...
  RooCategory *pid_test =
      static_cast<RooCategory *>(test->find("pid_category"));
...
void add_points_to_tree(map<TString, TTree *> &outputTrees,
                        const points_analyzed_t &pts, TString treeName) {
  map<TString, TTree *>::iterator it = outputTrees.find(treeName);
  if (it != outputTrees.end()) {
    cout << "Tree: " << treeName.Data() << " already exists. Won't write"
              << endl;
    return;
  }
...
void do_debug_run(global_config &config, StatisticsModel *full_model) {
  ...
  full_model->Update(config, config.mass_points[0],
                     config.log_coupling_points[0]);
  cout << "Mass = " << config.mass_points[0]
            << " logE = " << config.log_coupling_points[0]
            << " yield = " << full_model->GetTotalYield() << endl;
  /////////////////////////////////////////////////////////////////////////////
  /////////////////////// Define dummy observation/////////////////////////////
  /////////////////////////////////////////////////////////////////////////////

  RooDataSet observation(
      "obs", "obs", {*full_model->invariantMass, *full_model->pid_categories});
  ...
  if (config.useMuMu) {
    ...
    observation.add(
        RooArgSet(*full_model->invariantMass, *full_model->pid_categories));
  }
  observation.Print("v");
  ...
  if (config.tossingType == kFixToFit) {
    ...
    if (config.statType == kTevatron) {
      ...
      cout << "nll_sb = " << nll_sb_obs
                << " nll_bkg = " << fr_bonly_obs->minNll();
    ...
    full_model->mu_test->setVal(1.0);
    full_model->mu_test->setConstant(true);
    RooNLLVar nll_sb_var("nll_sb_var", "nll_sb_var", *sb_model, observation,
                         RooFit::Extended());
    cout << "nll_sb = " << nll_sb_var.getVal();
    q_obs = nll_sb_var.getVal();

    RooNLLVar nll_bkg_var("nll_bkg_var", "nll_bkg_var", *bkg_model, observation,
                          RooFit::Extended());
  ...
  for (Int_t iToy = 0; iToy < config.n_toys; ++iToy) {
    ...
    unique_ptr<RooDataSet> toys_sb{
        full_model->Generate(config, 1.0, sb_model, fitted_values_sb)};
  ...
  ds_ngen.tree()->Write("n_gen");
  double time_sb_toys = sw.RealTime();
  cout << "Done " << config.n_toys << " toys in " << time_sb_toys << " s"
            << endl;
  ...
  for (Int_t iToy = 0; iToy < config.n_toys; ++iToy) {
    ...
    unique_ptr<RooDataSet> toys{
        full_model->Generate(config, 0.0, bkg_model, fitted_values_bkg)};
    double q = full_model->ComputeQstatistic(config, *toys, params_init);
    q_toys_bonly.push_back(q);
    if (q_toys_bonly.back() >= q_obs ||
        fabs(q_toys_bonly.back() - q_obs) < 1E-3)
      n_higher_bkg += 1.0;
  }
  double time_bkg_toys = sw.RealTime() - time_sb_toys;
  cout << "Done " << config.n_toys << " toys in " << time_bkg_toys << " s"
            << endl;
  tree_debug->Fill();

  cout << "N higher q (s+b): " << n_higher_sb
            << " N higher q (nkg): " << n_higher_bkg << endl;
  double cls = 1.0;
...
void do_exclusion_run(global_config &config, StatisticsModel *full_model) {
  ...
  for (size_t iM = 0; iM < config.mass_points.size(); iM++) {
    ...
    for (size_t iE = 0; iE < config.log_coupling_points.size(); iE++) {
      ...
      cout << "Current point: mass = " << current_mass
                << " log(e) = " << current_loge
                << " Total yield = " << full_model->GetTotalYield()
                << endl;
      ...
      if (config.tossingType == kFixToFit) {
        ...
        if (config.statType == kLHC) {
          ...
          full_model->mu_test->setVal(1.0);
          full_model->mu_test->setConstant(false);
          unique_ptr<RooFitResult> fr_unconditional{
              sb_model->fitTo(
                  observation,
                  /*RooFit::GlobalObservables(*full_model->global_observarbles),*/
                  RooFit::Constrain(*full_model->constrained_parameters),
          ...
          double nll_uncond = fr_unconditional->minNll();
          cout << "nll_sb = " << nll_sb_obs
                    << " nll_uncond = " << nll_uncond;
          q_obs = 2.0 * (nll_sb_obs - nll_uncond);
        } else {
          cout << "nll_sb = " << nll_sb_obs
                    << " nll_bkg_obs = " << fr_bkg_obs->minNll();
        ...
        RooNLLVar nll_bkg_var("nll_bkg_var", "nll_bkg_var", *bkg_model,
                              observation, RooFit::Extended());
        cout << " nll_bkg = " << nll_bkg_var.getVal();
        q_obs = q_obs - nll_bkg_var.getVal();
        q_obs = 2.0 * q_obs;
      }
      ...
      for (Int_t iToy = 0; iToy < config.n_toys; ++iToy) {
        unique_ptr<RooDataSet> toys_sb{
            full_model->Generate(config, 1.0, sb_model, fitted_values_sb)};
        double q_toy =
            full_model->ComputeQstatistic(config, *toys_sb, params_init);
        if (q_toy >= q_obs || fabs(q_toy - q_obs) < 1E-3)
          n_higher_sb += 1.0;
      }
      double time_sb_toys = sw.RealTime();
      cout << "Done " << config.n_toys << " toys in "
                << time_sb_toys - t_start_sb << " s" << endl;
      ...
      for (Int_t iToy = 0; iToy < config.n_toys; ++iToy) {
        ...
        unique_ptr<RooDataSet> toys{
            full_model->Generate(config, 0.0, bkg_model, fitted_values_bkg)};
        double q_toy =
            full_model->ComputeQstatistic(config, *toys, params_init);

        if (q_toy >= q_obs || fabs(q_toy - q_obs) < 1E-3)
          n_higher_bkg += 1.0;
      }
      double time_bkg_toys = sw.RealTime() - time_sb_toys;
      cout << "Done " << config.n_toys << " toys in " << time_bkg_toys
                << " s" << endl;
...
void do_expectedband_run(global_config &config, StatisticsModel *full_model) {
  ...
  //     This is a bit uglier, but the thing has to run.
  ///
  cout << "============  Expected band run starts now ==============="
            << endl;
  TFile *output_exclusion = TFile::Open("output_exclusion.root", "RECREATE");
  TTree *tree_exclusion = new TTree("CLS", "CLS");
  ...
  for (size_t iMass = 0; iMass < config.mass_points.size(); iMass++) {
    ...
    for (size_t iE = 0; iE < config.log_coupling_points.size(); iE++) {
      ...
      full_model->fit_parameters->assignFast(*params_init);
      unique_ptr<RooDataSet> pseudo_data_aux{
          full_model->bkgModel_full->generate(*full_model->global_observarbles,
                                              1)};
      full_model->global_observarbles->assignFast(*pseudo_data_aux->get(0));

      unique_ptr<RooDataSet> pseudo_observation{
          full_model->bkgModel_full->generate(
	...
	pseudo_observation->addColumn(*pseudo_globalObsArg);
      ...
      full_model->ComputeQDistribution(config, full_model->sbModel_full,
                                       fitted_values_sb, params_init,
                                       q_distribution_sb);
      ...
      sw.Continue();
      cout << "q-distribution for s+b done in " << t_q_sb - t_start_point
                << " s" << endl;

      vector<double> q_distribution_bkg;
      full_model->ComputeQDistribution(config, full_model->bkgModel_full,
                                       fitted_values_bkg, params_init,
                                       q_distribution_bkg);
      ...
      sw.Continue();
      cout << "q-distribution for bkg done in " << t_q_bkg - t_q_sb << " s"
                << endl;
      ...
      for (int iToy = 0; iToy < config.n_toys; iToy++) {
        if (config.n_toys > 10) {
          if (iToy % partition == 0) {
            cout << "Doing pseudo-observation " << iToy << "/"
                      << config.n_toys << endl;
          }
        }

        full_model->fit_parameters->assignFast(*params_init);
        unique_ptr<RooDataSet> toy_aux{full_model->bkgModel_full->generate(
            *full_model->global_observarbles, 1)};
        full_model->global_observarbles->assignFast(*toy_aux->get(0));

        unique_ptr<RooDataSet> toy_observation{
            full_model->bkgModel_full->generate(
                {*full_model->invariantMass, *full_model->pid_categories},
                RooFit::Extended(), RooFit::Verbose(false))};
	...
	delete iter;
        ...
        double q = full_model->ComputeQstatistic(config, *toy_observation,
                                                 params_init);
        // compute CLS for this pseudo-observation
        vector<double>::iterator it_sb = upper_bound(
            q_distribution_sb.begin(), q_distribution_sb.end(), q, compare_q);
        double n_higher_sb =
            1.0 * config.n_toys - 1.0 * (it_sb - q_distribution_sb.begin());

        vector<double>::iterator it_bkg = upper_bound(
            q_distribution_bkg.begin(), q_distribution_bkg.end(), q, compare_q);
        double n_higher_bkg =
            1.0 * config.n_toys - 1.0 * (it_bkg - q_distribution_bkg.begin());
...
int main(int argc, char **argv) {
  ...
  if (config.useEE)
    full_model->AddChannel("ee", kGaussian, kLandau, 0.0094, 4.0);
  if (config.useMuMu)
    full_model->AddChannel("mumu", kGaussian, kBernstein, 0.0087, 1.13);
  ...
  TF1 *f1 = new TF1(
      "f1", "[0]/(TMath::Sqrt(2.0*TMath::Pi())*TMath::Log([1])*x) * "
