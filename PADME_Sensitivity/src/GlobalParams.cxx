...
void global_config::retrieve_input() {
  ...
  ee_yield["meson"] =
      static_cast<TH2D *>(dp_ee_yield->FindObjectAny("hNexp_90CL_ee_meson")
                              ->Clone("hYield_ee_meson"));
  ...
  mumu_yield["direct"] =
      static_cast<TH2D *>(dp_mumu_yield->FindObjectAny("hNexp_90CL_mumu_direct")
                              ->Clone("hYield_mumu_direct"));
  mumu_yield["direct"]->SetDirectory(0);

  mumu_yield["meson"] =
      static_cast<TH2D *>(dp_mumu_yield->FindObjectAny("hNexp_90CL_mumu_meson")
                              ->Clone("hYield_mumu_meson"));
...
void global_config::configure_run(const int &argc, char **argv) {
  ...
  if (argc == 1) {
    // use default
    for (Int_t iM = 1; iM <= hMSigma["mumu"]["direct"]->GetXaxis()->GetNbins();
         iM++) {
      mass_points.push_back(
          hMSigma["mumu"]["direct"]->GetXaxis()->GetBinCenter(iM));
    }
  } else if (argc == 2) {
  ...
  double min_log_coupling = -14.;
  double max_log_coupling = -6.;
  Int_t n_log_coupling =
      floor((max_log_coupling - min_log_coupling) / dlog_coupling) + 1;
