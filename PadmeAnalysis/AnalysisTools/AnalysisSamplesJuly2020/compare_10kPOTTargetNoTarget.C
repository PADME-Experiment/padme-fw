...
void compare_10kPOTTargetNoTarget(std::string hname, double scalef=-1, double xmin=-999, double xmax=999, 
...
//return;
  ...
  // myText(       0.2, 0.7 , 1, "nPOT=9.3#times10^{9}");
   ...
   t->AddEntry(hData,"10k POT/bunch of 150 ns target","ep");
   ...
   t->AddEntry(hsMC,"10k POT/bunch of 150 ns no target","l");
   t->SetTextAlign(12);
   t->SetBorderSize(0);
   t->SetFillStyle(0);
   t->SetFillColor(0);
  ...
  c->Print("AnalysisHisto10k_targetnotarget.pdf(");
   if(/*hname=="SS2gYEw_passDtDphiCogDsume"||hname=="ECal_NCluster" || */hname=="ECAL_MissingMassAnnihilation_10ns5CoGUnderPeakEThr90") c->Print("AnalysisHisto10k_targetnotarget.pdf)");
...
//return;
  ...
  // myText(       0.2, 0.7 , 1, "nPOT=9.3#times10^{9}");
   ...
   t->Draw();
   Int_t gg10=MakeDifference(hData, hsMC, minInt, maxInt);
   std::fstream fs;
   fs.open("ggCount_10k150ns.txt", std::fstream::in | std::fstream::out | std::fstream::app);
   fs<<hname.c_str() << "                " << gg10 << std::endl;
   fs.close();
   
  c->SaveAs(("outpng/"+hname+".png").c_str());
  c->Print("AnalysisHisto10k_targetnotarget.pdf(");
  //  c->Print("AnalysisHisto5k.pdf(");
   if(lastSet==1) c->Print("AnalysisHisto10k_targetnotarget.pdf)");
