...
void compare_15kPOTTargetNoTarget(std::string hname, double scalef=-1, double xmin=-999, double xmax=999, 
...
//return;
  ...
  // myText(       0.2, 0.7 , 1, "nPOT=9.3#times10^{9}");
   ...
   t->AddEntry(hData,"15k POT/bunch of 150 ns target","ep");
   ...
   t->AddEntry(hsMC,"15k POT/bunch of 150 ns no target","l");
   t->SetTextAlign(12);
   t->SetBorderSize(0);
   t->SetFillStyle(0);
   t->SetFillColor(0);
  ...
  c->Print("AnalysisHisto15k_targetnotarget.pdf(");
   if(/*hname=="SS2gYEw_passDtDphiCogDsume"||hname=="ECal_NCluster" || */hname=="ECAL_MissingMassAnnihilation_10ns5CoGUnderPeakEThr90") c->Print("AnalysisHisto15k_targetnotarget.pdf)");
...
//return;
  ...
  // myText(       0.2, 0.7 , 1, "nPOT=9.3#times10^{9}");
   ...
   t->Draw();
   Int_t gg15=MakeDifference(hData, hsMC, minInt, maxInt);
   std::fstream fs;
   fs.open("ggCount_15k150ns.txt", std::fstream::in | std::fstream::out | std::fstream::app);
   fs<<hname.c_str() << "                " << gg15 << std::endl;
   fs.close();
   
  c->SaveAs(("outpng/"+hname+".png").c_str());
  c->Print("AnalysisHisto15k_targetnotarget.pdf(");
  //  c->Print("AnalysisHisto5k.pdf(");
   if(lastSet==1) c->Print("AnalysisHisto15k_targetnotarget.pdf)");
