...
void compare_20kPOT(std::string hname, double scalef=-1, double xmin=-999, double xmax=999, 
	       ...
	       TFile* fData, TFile* fMC)
{
  ...
  */
  ...
  if (scalef<0) 
    {
      ...
      std::cout<<"data " << nPOTdata << " MC " << nPOTMC;
      ...
      Double_t fcorr=20000./Mean20;
      nPOTdata =nPOTdata*fcorr;
      scalef = nPOTdata/nPOTMC;
      std::cout<< " corrF " << fcorr << " new npotData " << nPOTdata <<" scalef "<< scalef << " MC should be " <<nPOTMC*scalef << std::endl;
    }
  ...
  if(hname=="nPOT"){
    std::cout<<"after scaling " << hData->GetBinContent(2) << " mc " << hsMC->GetBinContent(2) << std::endl;
  }
  double ymax = ymaxMC;
...
//return;
  ...
  c->Print("AnalysisHisto20k_compareBunchLenght.pdf(");
   if(/*hname=="SS2gYEw_passDtDphiCogDsume"||hname=="ECal_NCluster" || */hname=="ECAL_MissingMassAnnihilation_10ns5CoGUnderPeakEThr90") c->Print("AnalysisHisto20k_compareBunchLenght.pdf)");
