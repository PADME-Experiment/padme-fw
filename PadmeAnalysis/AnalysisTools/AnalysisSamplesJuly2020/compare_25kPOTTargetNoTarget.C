...
void compare_25kPOTTargetNoTarget(std::string hname, double scalef=-1, double xmin=-999, double xmax=999, 
...
//return;
   ...
   */
   ...
   std::ostringstream s;
   //   s<<"nPOT=" << nPOTdata;
   s<<"nPOT=" << nPOTdata << " x " <<  corrNPOT(fData, 25000);
  ...
  // myText(       0.2, 0.7 , 1, "nPOT=9.3#times10^{9}");
   ...
   t->AddEntry(hData,"25k POT/bunch of 150 ns target","ep");
   ...
   t->AddEntry(hsMC,"25k POT/bunch of 150 ns no target","l");
   t->SetTextAlign(12);
   t->SetBorderSize(0);
   t->SetFillStyle(0);
   t->SetFillColor(0);
  ...
  c->Print("AnalysisHisto25k_targetnotarget.pdf(");
  ...
  //if(/*hname=="SS2gYEw_passDtDphiCogDsume"||hname=="ECal_NCluster" || */hname=="ECAL_MissingMassAnnihilation_10ns5CoGUnderPeakEThr90") c->Print("AnalysisHisto5k.pdf)");
   if(/*hname=="SS2gYEw_passDtDphiCogDsume"||hname=="ECal_NCluster" || */hname=="ECAL_MissingMassAnnihilation_10ns5CoGUnderPeakEThr90") c->Print("AnalysisHisto25k_targetnotarget.pdf)");
...
//return;
  ...
  // myText(       0.2, 0.7 , 1, "nPOT=9.3#times10^{9}");
   ...
   t->Draw();
   Int_t gg25=MakeDifference(hData, hsMC, minInt, maxInt);
   std::fstream fs;
   fs.open("ggCount_25k150ns.txt", std::fstream::in | std::fstream::out | std::fstream::app);
   fs<<hname.c_str() << "                " << gg25 << std::endl;
   fs.close();
   
  c->SaveAs(("outpng/"+hname+".png").c_str());
  c->Print("AnalysisHisto25k_targetnotarget.pdf(");
  //  c->Print("AnalysisHisto5k.pdf(");
   if(lastSet==1) c->Print("AnalysisHisto25k_targetnotarget.pdf)");
