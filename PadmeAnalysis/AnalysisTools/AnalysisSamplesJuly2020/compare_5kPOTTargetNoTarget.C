...
void compare_5kPOTTargetNoTarget(std::string hname, double scalef=-1, double xmin=-999, double xmax=999, 
  ...
  if (scalef<0) 
    {
      ...
      std::cout<<"target ns nPOT: " <<nPOTdata <<" no target ns nPOT " <<nPOTMC << " scale factor " << scalef << std::endl;
	...
	std::cout<<"left " << IntegralLeft5 << "left no target " << IntegralLeft10 << std::endl;
      }
      std::cout<<"after use normDTime scale factor " << scalef << std::endl;
...
//return;
  ...
  c->Print("AnalysisHisto5k_targetnotarget.pdf(");
  ...
  //if(/*hname=="SS2gYEw_passDtDphiCogDsume"||hname=="ECal_NCluster" || */hname=="ECAL_MissingMassAnnihilation_10ns5CoGUnderPeakEThr90") c->Print("AnalysisHisto5k.pdf)");
   if(/*hname=="SS2gYEw_passDtDphiCogDsume"||hname=="ECal_NCluster" || */hname=="ECAL_MissingMassAnnihilation_10ns5CoGUnderPeakEThr90") c->Print("AnalysisHisto5k_targetnotarget.pdf)");
  ...
  if (scalef<0) 
    {
      ...
      double nPOTMC = hMCN->GetBinContent(2);
      scalef = nPOTdata/nPOTMC;
      std::cout<<"-*------target nPOT: " <<nPOTdata <<" target nPOT " <<nPOTMC << " scale factor " << scalef << std::endl;
      Bool_t normUsingDTime=true;
      if(normUsingDTime){
	TH1D * DTime = (TH1D*)fData->Get("ECAL_dTime_10ns20DegreeDeltaTheta");
	Double_t bin1_5=DTime->FindBin(-10);
	...
	scalef=IntegralLeft5/IntegralLeft10;
      ...
      std::cout<<"after use normDTime scale factor " << scalef << " target " << IntegralLeft5 <<" noTarget " <<IntegralLeft10 <<std::endl;
    }
...
//return;
  ...
  // myText(       0.2, 0.7 , 1, "nPOT=9.3#times10^{9}");
   ...
   t->Draw();
   Int_t gg5=MakeDifference(hData, hsMC, minInt, maxInt);
   std::fstream fs;
   fs.open("ggCount_5k150ns.txt", std::fstream::in | std::fstream::out | std::fstream::app);
   fs<<hname.c_str() << "                " << gg5 << std::endl;
   fs.close();
   
  c->SaveAs(("outpng/"+hname+".png").c_str());
  c->Print("AnalysisHisto5k_targetnotarget.pdf(");
  //  c->Print("AnalysisHisto5k.pdf(");
   if(lastSet==1) c->Print("AnalysisHisto5k_targetnotarget.pdf)");
