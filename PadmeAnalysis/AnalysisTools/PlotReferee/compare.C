...
void compare(std::string hname, double scalef=-1, double xmin=-999, double xmax=999, 
  ...
  if (scalef<0) 
    {
      ...
      //double nPOTMC = hMCN->GetEntries();
      TH1D* hPOTdisMC=(TH1D*)fMC->Get("NposInBunch_beam");
...
//return;
  ...
  //   ATLASLabel(0.2,0.8,"Work in progress");//,"Preliminary");
   ...
   myText(       0.2, 0.64 , 1, "E_{beam}=490 MeV, 23k e^{+}/ 150 ns bunch");
  ...
  c->SaveAs(("Plot_"+hname+".C").c_str());
  c->Print("AnalysisHisto.pdf(");
   if(/*hname=="SS2gYEw_passDtDphiCogDsume"||hname=="ECal_NCluster" || */hname=="ECAL_tgg_10ns1CoGUnderPeakEThr90") c->Print("AnalysisHisto.pdf)");
