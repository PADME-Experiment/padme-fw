  ...
  if (scalef<0) 
    {
      ...
      TH1D* hDN = (TH1D*)fData->Get("AnnMet_nPOT");
      //TH1D* hDN = (TH1D*)fData->Get("TargetBeam_nPOT");
      TH1D* hMCN = (TH1D*)fMC->Get("AnnMet_nPOT");
	...
	TH1D * DTime10 = (TH1D*)fMC->Get("ECAL_Dtgg_10ns_g1g2inFRin20DegThetaCut");
...
//return;
  ...
  // myText(       0.2, 0.7 , 1, "nPOT=9.3#times10^{9}");
   ...
   t->AddEntry(hData,"28k POT/bunch of 300 ns target","ep");
   ...
   t->AddEntry(hsMC,"28k POT/bunch of 300 ns no target","l");
   t->SetTextAlign(12);
   t->SetBorderSize(0);
   t->SetFillStyle(0);
   t->SetFillColor(0);
