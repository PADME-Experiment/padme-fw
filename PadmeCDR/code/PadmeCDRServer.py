...
class PadmeCDRServer:
    ...
    def __init__(self,daq_server,mode):

        # Get position of CDR main directory from PADME_CDR_DIR environment variable
        # Default to current dir if not set
        self.cdr_dir = os.getenv('PADME_CDR_DIR',".")

        # Get name of data server
        self.daq_server = daq_server

        # Redefine print to send output to log file
        self.log_file = "%s/log/PadmeCDRServer_%s.log"%(self.cdr_dir,self.daq_server)
        sys.stdout = Logger(self.log_file)
        sys.stderr = sys.stdout
        if mode == "i": sys.stdout.interactive = True

        print ""
        print "### PadmeCDRServer for %s ### Initializing ###"%self.daq_server
        print ""

        # Define file to store list of files with transfer errors
        self.transfer_error_list_file = "%s/log/transfer_error_%s.list"%(self.cdr_dir,self.daq_server)

        # Create lock file
        self.lock_file = "%s/run/lock_%s"%(self.cdr_dir,self.daq_server)
    ...
    def main_loop(self):
        ...
        print ""
        print "### PadmeCDRServer ### Start copying from %s ###"%self.daq_server
        print ""

        # SFTP URL for rawdata on this data server
        daq_sftp = "sftp://%s%s"%(self.daq_server,self.daq_path)

        # SSH syntax to execute a command on the DAQ data server
        daq_ssh = "ssh -i /home/%s/.ssh/%s -l %s %s"%(self.cdr_user,self.daq_keyfile,self.daq_user,self.daq_server)
        ...
        while True:
            ...
            print "Getting list of raw data files for year %s on %s"%(self.year,self.daq_server)
            daq_list = []
            ...
            for line in self.run_command("%s \'( cd %s/%s; find -type f -name \*.root | sed -e s+\./++ )\'"%(daq_ssh,self.daq_path,self.data_dir)):
            ...
            for rawfile in (full_list):
                ...
                if ( rawfile in daq_list and not rawfile in self.lnf_list ):
                    ...
                    if ( a32_src != "" and a32_dst != "" and a32_src == a32_dst ):
                        ...
                        with open(self.transfer_error_list_file,"a") as telf: telf.write("%s\n"%rawfile)
