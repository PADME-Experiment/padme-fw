...
MMegaIonizations::MMegaIonizations(G4ThreeVector start, G4ThreeVector end, G4double dE)
{
    ...
    for(int i = 0; i < fNionizations; i++){ // generates drift electrons in the step
    ...
    if(geo->GetReadoutType() == "strips") ComputeStripID();
    else if(geo->GetReadoutType() == "pads") ComputePadID();
...
void MMegaIonizations::ComputeStripID(){
    ...
    // add gaussian smearing for time and space resolution
    // put gain weighting in digitizer

    for(int i = 0; i < fNionizations; i++){
        G4double x = fIonizations[i].x();
        G4double y = fIonizations[i].y();
        G4double z = fIonizations[i].z();
...
void MMegaIonizations::ComputePadID(){
