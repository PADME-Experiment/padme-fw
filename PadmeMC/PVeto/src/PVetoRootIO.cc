...
PVetoRootIO::PVetoRootIO() : MCVRootIO(G4String("PVeto"))
{
  ...
  if (fVerbose) G4cout << "PVetoRootIO: Initialized" << G4endl;
...
{
  if (fVerbose) G4cout << "PVetoRootIO: Executing End-of-Run procedure" << G4endl;
}
...
void PVetoRootIO::SaveEvent(const G4Event* eventG4)
{
  ...
  if (fHitsEnabled) {
	...
	if (fVerbose>=2) G4cout << "PVetoRootIO: Found hits collection " << HCname << G4endl;
  ...
  if (fDigisEnabled) {
	...
	if (fVerbose>=2) G4cout << "PVetoRootIO: Found digi collection " << DCname << G4endl;
