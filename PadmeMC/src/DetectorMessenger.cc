...
{ 
  ...
  fMMReadoutCmd = new G4UIcmdWithAString("/Detector/MMReadoutType",this);
  fMMReadoutCmd->SetGuidance("Defines layout of MM Readout Planes.");
  fMMReadoutCmd->SetGuidance("Possible choices are:");
  fMMReadoutCmd->SetGuidance("strips: readout electrode is segmented in strips.");
  fMMReadoutCmd->SetGuidance("pads: readout electrode is segmented in pads");
  fMMReadoutCmd->SetParameterName("Readout Type",false);
  fMMReadoutCmd->SetCandidates("strips pads");
