...
//
//
// $Id: MagneticField.cc,v 1.2 2014/01/22 16:37:06 veni Exp $
//
//  
...
void MyBField::GetFieldValue(const double point[3],double *BValue) const
{
  double z,B0;
  z = point[2];
  if(point[2]<0) z=-point[2];
  ...
  BValue[0]=(-MagneticField*tesla)/0.365*B0;
  BValue[1]=0.;
  BValue[2]=0.;
}
  ...
  :  fChordFinder(0), fLocalChordFinder(0), fStepper(0)
{
  ...
  if(IsBFieldON==0){
    fLocalMagneticField = new G4UniformMagField(G4ThreeVector(0.0, 
    							      0.0,                 
    							      0.0));
  }else if(IsBFieldON==1){ 
    fLocalMagneticField = new G4UniformMagField(G4ThreeVector(-MagneticField*tesla, 
							      0.0,                 
							      0.0  ));
  }else{
    fLocalMagneticField = new MyBField();
  }
