...
{
  SACCollection = new SACHitsCollection
...
G4bool SACSD::ProcessHits(G4Step* aStep,G4TouchableHistory*)
{
  ...
  if(CurrentProcess != 0) {
    if(CurrentProcess->GetProcessName()== "Transportation" && track->GetVolume()->GetName()=="SAC") {
      ...
      newHit -> SetSACNb(aStep->GetPreStepPoint()->GetTouchableHandle()->GetCopyNumber());
