...
void DigitizerChannelECal::PrepareDebugHistos(){
  ...
  hListCal->Add(hDiffWavetemplate_0_10 = new TH1F("hDiffWavetemplate_0_10", "hDiffWavetemplate_0_10",100, -5, 5));
  hListCal->Add(hDiffWavetemplate_10_30 = new TH1F("hDiffWavetemplate_10_30", "hDiffWavetemplate_10_30",100, -5, 5));
  hListCal->Add(hDiffWavetemplate_30_50 = new TH1F("hDiffWavetemplate_30_50", "hDiffWavetemplate_30_50",100, -5, 5));
  hListCal->Add(hDiffWavetemplate_50_100 = new TH1F("hDiffWavetemplate_50_100", "hDiffWavetemplate_50_100",100, -5, 5));
  hListCal->Add(hDiffWavetemplate_100_150 = new TH1F("hDiffWavetemplate_100_150", "hDiffWavetemplate_100_150",100, -5, 5));
  hListCal->Add(hDiffWavetemplate_150_200 = new TH1F("hDiffWavetemplate_150_200", "hDiffWavetemplate_150_200",100, -5, 5));
  hListCal->Add(hDiffWavetemplate_200_250 = new TH1F("hDiffWavetemplate_200_250", "hDiffWavetemplate_200_250",100, -5, 5));
  hListCal->Add(hDiffWavetemplate_250_300 = new TH1F("hDiffWavetemplate_250_300", "hDiffWavetemplate_250_300",100, -5, 5));
...
void DigitizerChannelECal::DrawMeanWave(UShort_t iDer, Double_t& FirstEnergy, Double_t& FirstTime, Double_t& SecondEnergy,Double_t& SecondTime,Double_t& ThirdEnergy,Double_t& ThirdTime, Bool_t& SecondHit, Bool_t& ThirdHit) {
  ...
  if(fGlobalMode->GetGlobalDebugMode()){
    ...
    if(energyFirstHitFromTemplate<10.){for(int jj=0; jj< DiffVec.size(); jj++){if(fabs(DiffVec.at(jj))<sqrt(TempWave.at(jj)))hDiffWavetemplate_0_10->Fill(DiffVec.at(jj));}}
    if(energyFirstHitFromTemplate>10. && energyFirstHitFromTemplate<30. ){for(int jj=0; jj< DiffVec.size(); jj++)  {if(fabs(DiffVec.at(jj))<sqrt(TempWave.at(jj)))hDiffWavetemplate_10_30->Fill(DiffVec.at(jj));}}
    if(energyFirstHitFromTemplate>30. && energyFirstHitFromTemplate<50. ){for(int jj=0; jj< DiffVec.size(); jj++)  {if(fabs(DiffVec.at(jj))<sqrt(TempWave.at(jj)))hDiffWavetemplate_30_50->Fill(DiffVec.at(jj));}}
    if(energyFirstHitFromTemplate>50. && energyFirstHitFromTemplate<100. ){for(int jj=0; jj< DiffVec.size(); jj++) {if(fabs(DiffVec.at(jj))<sqrt(TempWave.at(jj)))hDiffWavetemplate_50_100->Fill(DiffVec.at(jj));}}
    if(energyFirstHitFromTemplate>100. && energyFirstHitFromTemplate<150. ){for(int jj=0; jj< DiffVec.size(); jj++){if(fabs(DiffVec.at(jj))<sqrt(TempWave.at(jj)))hDiffWavetemplate_100_150->Fill(DiffVec.at(jj));}}
    if(energyFirstHitFromTemplate>150. && energyFirstHitFromTemplate<200. ){for(int jj=0; jj< DiffVec.size(); jj++){if(fabs(DiffVec.at(jj))<sqrt(TempWave.at(jj)))hDiffWavetemplate_150_200->Fill(DiffVec.at(jj));}}
    if(energyFirstHitFromTemplate>200. && energyFirstHitFromTemplate<250. ){for(int jj=0; jj< DiffVec.size(); jj++){if(fabs(DiffVec.at(jj))<sqrt(TempWave.at(jj)))hDiffWavetemplate_200_250->Fill(DiffVec.at(jj));}}
    if(energyFirstHitFromTemplate>250. && energyFirstHitFromTemplate<300. ){for(int jj=0; jj< DiffVec.size(); jj++){if(fabs(DiffVec.at(jj))<sqrt(TempWave.at(jj)))hDiffWavetemplate_250_300->Fill(DiffVec.at(jj));}}
  ...
  */
  ...
  //std::cout<<"First hit " << fTimeSin << " possible second hit at " << SecondBinMax << " max Derivative " << maxValDerivativeDiffForSecondHit << std::endl;
  if(maxValDerivativeDiffForSecondHit>fVThrToDefineHit && fabs(DiffFirstSecondHit)>fDTimeForHitInSameCh){   //I'm cutting on the value of the waveform at the maximumderivative time of diffvec
    ...
    //std::cout<<"In Draw.....SeconhHitEnergy " << SecondEnergy <<" time "<< TimeSecondHit<< std::endl;
    ...
    //std::cout<<"finished to filling histos " << std::endl;
    //std::cout<<"thirdPosible amplitude " << maxValDerivativeDiffForThirdHit << " at time " << TimeThirdHit << std::endl;
    if(maxValDerivativeDiffForThirdHit>fVThrToDefineHit &&  fabs(DiffFirstThirdHit)>fDTimeForHitInSameCh &&  fabs(DiffSecondThirdHit)>fDTimeForHitInSameCh ){   //I'm cutting on the value of the waveform at the maximumderivative time of diffvec
      //std::cout<<"In Draw.....ThirdHit time " << TimeThirdHit << std::endl;
      ...
      //std::cout<<"DiffVecSecondHit (for 3rd hit) max "<< maxWave3 << std::endl;
...
Double_t DigitizerChannelECal::MakeDerivativeAndTakeMaxTime(Int_t iDer, Int_t nsmooth, std::vector<Double_t> wave, Double_t& maxValDerivative){
  ...
  if(end>900)end=900;
  for(int ll=0;ll<1001;ll++){
    if(ll< wave.size() && wave.at(ll)<0.)wave.at(ll)=0.;//io 22/07/21 to improve the third hit identification for MC
...
void DigitizerChannelECal::DrawMeanSaturatedWave(UShort_t iDer, Double_t& FirstEnergy, Double_t& FirstTime, Double_t& SecondEnergy,Double_t& SecondTime,Double_t& ThirdEnergy,Double_t& ThirdTime, Bool_t& SecondHit, Bool_t& ThirdHit) {
  ...
  if(!secondHitSaturated){
    ...
    Double_t SecondBinMax=MakeDerivativeAndTakeMaxTime(iDer,nsmooth, DiffVec, maxValDerivativeDiffForSecondHit);
    Double_t TimeSecondHit= SecondBinMax*fTimeBin;  
    //std::cout <<"DrawMeanSaturatedWave... DiffVec at maxDerivative2 " << maxValDerivativeDiffForSecondHit << " from vec " << DiffVec.at(SecondBinMax) << " second bin max " << SecondBinMax << " time " << TimeSecondHit << std::endl;
	...
	enCorrection12 =  CalcChargeSin(MaxBin, TempWave)/15.;
      ...
      //<<"energySecondHitFromTemplate "<< energySecondHitFromTemplate << std::endl;
      if(fGlobalMode->GetGlobalDebugMode()) {
	//std::cout<<"i'm filling second template " << std::endl;
	for(int ll=0; ll<TempWave.size();ll++) histo4->SetBinContent(ll, TempWave.at(ll));
	hChargeSecondHit->Fill(chargeSecondHitFromTemplate);
	hEnergySecondHit->Fill(energySecondHitFromTemplate);
      }
    ...
    SecondEnergy=energySecondHitFromTemplate-enCorrection23;
    ThirdEnergy=energyThirdHitFromTemplate;
    //std::cout<<"energy1 " << FirstEnergy << " hit2 " << SecondEnergy << " hit3 " <<ThirdEnergy << std::endl;
    /*if(SecondHit){
