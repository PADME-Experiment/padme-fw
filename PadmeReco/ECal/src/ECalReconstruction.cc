...
struct by_time_cry
{
  bool operator()(TimeChIdx const &a, TimeChIdx const &b) const noexcept
  {// { if (a.digiChannel<b.digiChannel) return true;
  //   if (node1.y == node2.y) return node1.x < node2.x;

  //   return false;
    
    if(a.digiChannel==b.digiChannel){
    return a.digiTime < b.digiTime;
    }else{
      return a.digiChannel < b.digiChannel;
    }
  }
};
...
void ECalReconstruction::HistoInit()
{
  ...
  AddHisto("ECALHitEMC", new TH1F("ECALHitEMC", "ECALHitEMC", 550, 0, 550));
  AddHisto("ECALHitDT_MC", new TH1F("ECALHitDT_MC", "ECALHitDT_MC", 300, 0, 300));
  AddHisto("ECALHitEvsDT_MC", new TH2F("ECALHitEvsDT_MC", "ECALHitEvsDT_MC", 300, 0, 300, 300, 0, 300));
  AddHisto("ECALNSndHitvsch_MC", new TH2F("ECALNSndHitvsch_MC", "ECALNSndHitvsch_MC", 30,0,30,30,0,30));
  AddHisto("ECALNSndHitvsch_MC_moreHit", new TH2F("ECALNSndHitvsch_MC_moreHit", "ECALNSndHitvsch_MC_moreHit", 30,0,30,30,0,30));
...
void ECalReconstruction::ProcessEvent(TRawEvent *rawEv)
{
  ...
  if (fChannelCalibration)
    ((ECalCalibration *)fChannelCalibration)->PerformCalibration(GetRecoHits(), rawEv);
  if (fGeometry)
    fGeometry->ComputePositions(GetRecoHits());

  // from Hits to Clusters
  ClearClusters();
  BuildClusters();
  if (fChannelCalibration)
    fChannelCalibration->PerformCalibration(GetClusters());
...
bool ECalReconstruction::TriggerToBeSkipped()
{
  ...
  if (GetGlobalRecoConfigOptions()->IsPedestalMode() && !(GetTriggerProcessor()->IsAutoTrigger()))
    return true;
  if (GetGlobalRecoConfigOptions()->IsCosmicsMode() && !(GetTriggerProcessor()->IsCosmicsTrigger()))
    return true;
  return false;
}
...
void ECalReconstruction::BuildHits(TRawEvent *rawEv)
{
  ...
  for (Int_t iBoard = 0; iBoard < nBoards; iBoard++)
  {
    ...
    for (unsigned ich = 0; ich < ADC->GetNADCChannels(); ich++)
    {
      ...
      if (ChID == -1)
        continue; // Skip channels not used by ECal (e.g. LeadGlass and Cosmics)
...
void ECalReconstruction::AnalyzeEvent(TRawEvent *rawEv)
{
  ...
  for (unsigned int iHit1 = 0; iHit1 < Hits.size(); ++iHit1)
  {
    ...
    if (ix > 14 && iy > 14)
      q1 += Hits[iHit1]->GetEnergy();
    if (ix < 14 && iy < 14)
      q3 += Hits[iHit1]->GetEnergy();
    if (ix < 14 && iy > 14)
      q2 += Hits[iHit1]->GetEnergy();
    if (ix > 14 && iy < 14)
      q4 += Hits[iHit1]->GetEnergy();
  }
...
Int_t ECalReconstruction::FindSeed(Int_t nele, Int_t *Used, Double_t *Ene)
{
  ...
  for (UShort_t i = 0; i < nele; i++)
  {
    ...
    if (fMax < Ene[i] && Used[i] == 0 && Ene[i] > fClEnThrForSeed)
    { // ene cut to check
      fMax = Ene[i];
      iMax = i;
    }
  ...
  if (iMax > 1024)
    std::cout << "N hits " << nele << " " << iMax << " ene " << fMax << std::endl;
  return iMax;
}
...
Int_t ECalReconstruction::IsSeedNeig(Int_t seedID, Int_t cellID)
{
  ...
  if (abs(SeedRow - CellRow) <= fClDeltaCellMax && abs(SeedCol - CellCol) <= fClDeltaCellMax)
    IsNeig = 1;
...
void ECalReconstruction::BuildClusters()
{
  ...
  if (fClusterizationAlgo == 1)
    ECalReconstruction::BuildSimpleECalClusters();
  else if (fClusterizationAlgo == 2)
    PadmeVReconstruction::BuildClusters();
  else if (fClusterizationAlgo == 3 || fClusterizationAlgo == 4)
  { // use island(3) or Radius(4) algo
...
void ECalReconstruction::BuildSimpleECalClusters()
{
  ...
  while (iMax > -1)
  {
    ...
    for (unsigned int iHit1 = 0; iHit1 < Hits.size(); ++iHit1)
    {
      ...
      {
        if (NCry == NMaxHitsInCl)
          std::cout << "INFO: ECalReconstruction:: Maximum n. of Hits in this cluster reached (" << NMaxHitsInCl << "); go to next seed, if any" << std::endl;
        continue;
      }
      if (fabs(cTime[iHit1] - SdTime[NSeeds]) < fClDeltaTime && cUsed[iHit1] == 0 && IsSeedNeig(SdCell[NSeeds], cChID[iHit1]) == 1)
      {
        ...
        if (SdCell[NSeeds] != cChID[iHit1] && cEnergy[iHit1] > 10.)
          GetHisto("ECALHitTDiff")->Fill((cTime[iHit1] - SdTime[NSeeds]));
        NCry++;
        HitUsed++;
...
void ECalReconstruction::ConvertMCDigitsToRecoHitsWave(TMCVEvent *tEvent, TMCEvent *tMCEvent)
{
  ...
  double ys = fComulativeMax*(ydc-minTime)/width;*/
      ...
      if (xs < -45.)
        std::cout << "old time " << digiTime << " ys " << ys << " isbin " << isbin << " new Time " << xs << std::endl;
    ...
    for (int j = 0; j < hitArrayInCrystal.size(); j++)
    {
      ...
      { // let's see the correlation vmxVSenergy
        ...
        for (int i = 0; i < 1024; i++)
        {
          ...
          if (maxTempVec1 < tempVec1.at(i))
            maxTempVec1 = tempVec1.at(i);
        }
        // extract the charge
...
void ECalReconstruction::ConvertMCDigitsToRecoHits(TMCVEvent *tEvent, TMCEvent *tMCEvent){
  ...
  fHits.clear();
  Int_t idigi=0;
  Int_t ndigi = tEvent->GetNDigi();

  while (idigi < ndigi)
    {
      TMCVDigi *digi = tEvent->Digi(idigi);
      int i1 = digi->GetChannelId() / 100;
      int i2 = digi->GetChannelId() % 100;
      // Drop  Broken SU in Ecal
      Bool_t BrokenSU = SimulateBrokenSU(i2, i1);
      // digi in broken units removed
      if (BrokenSU) {
        tEvent->RemoveDigi(idigi);
        ndigi = tEvent->GetNDigi();
        continue;
        }

      //smear in time and energy
     ...
     std::vector<TimeChIdx> vectDigi;
     for (Int_t i = 0; i < tEvent->GetNDigi(); ++i)
     {
      TMCVDigi *digi = tEvent->Digi(i);
      TimeChIdx tci;
      tci.digiChannel = digi->GetChannelId();
      tci.digiTime = digi->GetTime();
      tci.digiIndex = i;
      // std::cout<<"i: "<<i<<" tci.digiTime: "<<tci.digiTime<< "tci.digiChannel: "<<tci.digiChannel<<std::endl;
      vectDigi.push_back(tci);
      }
      

      std::sort(vectDigi.begin(), vectDigi.end(), by_time_cry());
      // for(std::vector<TimeChIdx>::iterator iter = vectDigi.begin(); iter != vectDigi.end(); ++iter){
      // // std::cout<<"i sorted: "<<iter->digiIndex<<" tci.digiTime: "<<iter->digiTime<< "tci.digiChannel: "<<iter->digiChannel<<std::endl;
      // }



  if(fMultihitForMC==1){
    MultiHit1Algo(vectDigi, tEvent);
    }else if(fMultihitForMC==2){
    MultiHit2Algo(vectDigi, tEvent);
    }else if(fMultihitForMC==-2){
    MultiHitminus2Algo(vectDigi, tEvent);
    }else{
    MultiHit0Algo(vectDigi, tEvent);
    }
...
void ECalReconstruction::MultiHit1Algo(std::vector<TimeChIdx> vsorteddigi, TMCVEvent* tEvent){
  for(std::vector<TimeChIdx>::iterator iter = vsorteddigi.begin(); iter != vsorteddigi.end(); ++iter){
      TMCVDigi* digi = tEvent->Digi(iter->digiIndex);
...
void ECalReconstruction::MultiHit2Algo(std::vector<TimeChIdx> vsorteddigi, TMCVEvent* tEvent){
  Bool_t samechannel = true;
  Int_t oldchannel = -1, maxHitPerChannel=3, nRecoHitPerChannel=0;
  Double_t SndHitEThre= 10; //MeV //DA SPOSTARE DA QUA E METTERE NEL CONFIG FILE
  for(std::vector<TimeChIdx>::iterator iter = vsorteddigi.begin(); iter != vsorteddigi.end(); ++iter){
    if(iter->digiChannel != oldchannel && oldchannel!= -1){
      samechannel = false;
    ...
    if(!samechannel){
      ...
      GetHisto("ECALHitTimeDistribution")->Fill(digi->GetTime());
      if(oldchannel!= -1){
        TMCVDigi* digiold = tEvent->Digi((iter-1)->digiIndex);
        int i3 = digiold->GetChannelId() / 100;
        int i4 = digiold->GetChannelId() % 100;
        if(nRecoHitPerChannel>1){
        GetHisto("ECALNSndHitvsch_MC_moreHit")->Fill(i3,i4);
        }
        GetHisto("ECALNSndHitvsch_MC")->Fill(i3,i4);


      }
      nRecoHitPerChannel=1;
      //std::cout<<" no same ch: "<<digi->GetChannelId()<<" nhit: "<<nRecoHitPerChannel<<" index: "<<iter->digiIndex<< std::endl;
      oldchannel = iter->digiChannel;
    }else{
      TMCVDigi* digi = tEvent->Digi(iter->digiIndex);
      TMCVDigi* digiold = tEvent->Digi((iter-1)->digiIndex);
      //std::cout<<"same ch: "<<digi->GetChannelId()<<" nhit: "<<nRecoHitPerChannel<<" index: "<<iter->digiIndex<<" energy: "<<digi->GetEnergy()<< std::endl;
      GetHisto("ECALHitEMC")->Fill(digi->GetEnergy());
      GetHisto("ECALHitDT_MC")->Fill((digi->GetTime()-digiold->GetTime()));
      GetHisto("ECALHitEvsDT_MC")->Fill((digi->GetTime()-digiold->GetTime()),digi->GetEnergy());
      

      if (nRecoHitPerChannel > maxHitPerChannel) continue;
        // TMCVDigi* digi = tEvent->Digi(iter->digiIndex);
        // nRecoHitPerChannel++;
        if(digi->GetEnergy()<SndHitEThre) continue;
        TRecoVHit *Hit = new TRecoVHit();
        // @reconstruction level, the ECal ChIds are XXYY, while in MC they are YYXX
        int i1 = digi->GetChannelId() / 100;
        int i2 = digi->GetChannelId() % 100;
        ...
        fHits.push_back(Hit);
        nRecoHitPerChannel++;
        // std::cout<<"same ch: "<<digi->GetChannelId()<<" nhit: "<<nRecoHitPerChannel<<" index: "<<iter->digiIndex<< std::endl;
        GetHisto("ECALHitTimeDistribution")->Fill(digi->GetTime());
    }
...
//     delete fHits[i];
//   fHits.clear();
//   std::cout << "esisto dopo il return" << std::endl;
...
//       // Drop  Broken SU in Ecal
//       Bool_t BrokenSU = SimulateBrokenSU(i2, i1);
//       // std::cout << "esisto e  sto in fMultihit" << std::endl;

//       if (BrokenSU)
//         continue;
//       // effective threshold cutting noisy wf in 2019 data
//       if (digi->GetEnergy() < fClEnThrForHit)
//         continue; // fixed
//       TRecoVHit *Hit = new TRecoVHit();
//       // @reconstruction level, the ECal ChIds are XXYY, while in MC they are YYXX
//       int chIdN = i2 * 100 + i1;
...
//     std::sort(hitArrayInCrystal.begin(), hitArrayInCrystal.end(), by_time());
//     double energy, thitemax;
//     if (fMultihitForMC == 2)
//     {

//        //Temporary patched for smearing with MultiHit =2 //8/6/2024 Di Meco Raggi
//       energy = hitArrayInCrystal[0].digiEnergy;
//       thitemax = hitArrayInCrystal[0].digiTime;
//       //Double_t sigma = 2.32421e-01 + 4.39133e-02 * energy + 2.13789e-05 * energy * energy;
//       Double_t sigmaEovE = 1./TMath::Sqrt(energy*200); // photo statistics
//       Double_t sigma = energy*sigmaEovE; // photo statistics
...
//       Double_t sigmat = 0.9; //=1.4;  // MeV- from Single positron run; 0.8 from gg signal
//       Double_t DetTime = r->Gaus(0., sigmat);
//       thitemax += DetTime;
...
//       int maxLoop = 3;
//       if (hitArrayInCrystal.size() < 3)
//         maxLoop = hitArrayInCrystal.size();
//       for (Int_t dincr = 1; dincr < maxLoop; ++dincr)
//       {
//         if (hitArrayInCrystal[dincr].digiEnergy < 20.)
//           continue;
...
//     //   }
//     // }
//     // if (fDeteriorateHitEnResolution)
//     // { // MC Resolution 0.1, data resolution 1.2 (gg events)
//     //   // Double_t sigma=3.3;
...
//     //   std::cout << "Smearing applied in E " << std::endl;
//     //   Double_t sigma = 2.32421e-01 + 4.39133e-02 * energy + 2.13789e-05 * energy * energy;
//     //   // if(energy<200)sigma=3.3; // MeV
//     //   // else sigma=50.;  // MeV
//     //   Double_t DetEn = r->Gaus(0., sigma);
//     //   energy += DetEn;
//     // }
//     // if (fDeteriorateHitTimeResolution)
//     // { // MC Resolution 0.1, data resolution 1.2 (gg events)
//     //   std::cout << "Smearing applied in t " << std::endl;

//     //   Double_t sigma = 0.9; //=1.4;  // MeV- from Single positron run; 0.8 from gg signal
...
//     if (energy < 0.)
//       continue;
//     std::cout<<"Energy dopo continue:"<<energy<<std::endl;
//     Hit->SetEnergy(energy);
//     Hit->SetTime(thitemax);
//     fHits.push_back(Hit);
//   }
