...
ECalMLCalibHandler* ECalMLCalibHandler::fInstance = 0;

ECalMLCalibHandler* ECalMLCalibHandler::GetInstance()
{
  if ( fInstance == 0 ) { fInstance = new ECalMLCalibHandler(); }
  return fInstance;
}
...
ECalMLCalibHandler::ECalMLCalibHandler()
{
  // default list file
  fList = "config/Calibration/ECalMLEnergyCalibTimeIntervals.txt";
...
void ECalMLCalibHandler::Initialise()
{
  ...
  if(listfile.is_open()){ // open list file and fill corresponding map
    ...
    std::cout<<"\nWARNING!!! File "<<fList<<" with ECalML energy calibration "
...
void ECalMLCalibHandler::SetTimeInterval(TTimeStamp T_ev)
{
  ...
  if( (fCurTStart <= T_ev) && (T_ev <= fCurTEnd) ){ // current time interval is ok
	...
	std::cout<<"Setting ECalML energy calibration file to "<<FM_it->second
...
void ECalMLCalibHandler::SetCalibVal(std::string filename)
{
  ...
  if(calibfile.is_open()){
    ...
    std::cout<<"WARNING!!! Impossible to open ECalML energy calibration file "<<filename
	     <<". Calibration constants will be set to 0"<<std::endl;
