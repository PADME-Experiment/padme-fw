...
ECalMLCrystal::ECalMLCrystal(Int_t ix ,Int_t iy)
{
  ...
  Double_t calFrontFaceCenterZ = para->GetECalMLFrontFacePosZ();

  Int_t calNCryX = para->GetECalMLNCols();
  Int_t calNCryY = para->GetECalMLNRows();
...
ECalMLCrystal::~ECalMLCrystal()
{;}

void ECalMLCrystal::Print()
{
  std::cout << "ECalMLCrystal " << fXi << " " << fYi << " Status " << fStatus << " Energy " << fEnergy
