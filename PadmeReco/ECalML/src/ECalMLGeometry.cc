...
ECalMLGeometry::ECalMLGeometry()
  : PadmeVGeometry()
{
  std::cout<<"ECalMLGeometry being created ............"<<std::endl ;
  
}


void ECalMLGeometry::Init(PadmeVRecoConfig *cfg, RecoVChannelID *chIdMgr)
{
  ...
  fECalMLFrontFacePosZ = (double)cfg->GetParOrDefault("GEOMETRY","ECalMLFrontFacePosZ",3000.);
  ...
  float ECalMLZdistance = fECalMLFrontFacePosZ+0.5*fCrystalSizeZ;

 
  if (fStep1ChLocalX!=PitchX)
 {
  std::cout<<" WARNING!!! The ECalML pitch X may be wrong! "<<std::endl ;
 }

 if (fStep1ChLocalY!=PitchY)
 {
  std::cout<<" WARNING!!! The ECalML pitch Y may be wrong! "<<std::endl ;
 }
 
 if (fLocOzinPadmeFrame!=ECalMLZdistance)
 {
  std::cout<<" WARNING!!! The ECalML Z distance from 0 may be wrong! "<<std::endl ;
 }

}

TVector3  ECalMLGeometry::LocalPosition(Int_t chId)
{
