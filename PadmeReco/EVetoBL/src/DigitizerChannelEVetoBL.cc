...
void DigitizerChannelEVetoBL::PrintConfig(){
  //  std::cout << "Signal width: " << fSignalWidth << " samples" << std::endl;
  //  std::cout << "fUseAbsSignals: " << fUseAbsSignals << std::endl;  
}

DigitizerChannelEVetoBL::~DigitizerChannelEVetoBL(){
  if(fGlobalMode->IsPedestalMode() || fGlobalMode->GetGlobalDebugMode() || fSaveAnalog){
    SaveDebugHistos();
  }
}

void DigitizerChannelEVetoBL::Init(GlobalRecoConfigOptions *gMode, PadmeVRecoConfig *cfg){
  ...
  fTotalAnalogs = cfg->GetParOrDefault("Output","TotalAnalogs",90); //Beth 23/2/22: total number of analog signals to write to EVetoBLRecoAn.root

  fDigiFileOut = cfg->GetParOrDefault("Output","DigiFileOut","EVetoBLRecoAn.root");
...
Double_t DigitizerChannelEVetoBL::CalcPedestal() {  //get NSamples from data card
  fPed = 0.;
  fNPedSamples = 0;
...
Double_t DigitizerChannelEVetoBL::ZSupHit(Float_t Thr, UShort_t NAvg) {// NAvg = 1000, Thr by default is fZeroSuppression = 5. This parameter isn't in the config file, probably it should be?
...
Double_t DigitizerChannelEVetoBL::CalcChaTime(std::vector<TRecoVHit *> &hitVec){//copied and modified from 211019-padmefw, Beth 18/2/22. 22/2/22 Made it independent of pedestal and iMax
  RawGetMax=0;
  DerivGetMax=0;
  ...
  for (UShort_t ii = 0 ; ii != index.size() ; ii++) {
    ...
    Hit = new TRecoVHit();
    //    std::cout<<SetEVetoBLT0()<<std::endl;
    Hit->SetTime(tDerivSortHitVec[ii]-SetEVetoBLT0());//SetEVetoBLT0 gives the time offset of the channel
...
void DigitizerChannelEVetoBL::PrepareDebugHistos(){ //Beth 20/10/21 copied from 190917padme-fw, in turn copied from Mauro's DigitizerChannelECal structure to create a set of debug histograms that are produced only in debug mode
  ...
  hNegativeEnergyCha = new TH1F("hNegativeEnergyCha",";EVetoBLChannelID;",96,0,96);
...
void DigitizerChannelEVetoBL::SaveDebugHistos(){
...
void DigitizerChannelEVetoBL::Reconstruct(std::vector<TRecoVHit *> &hitVec){  //using TSpectrum
  if(fGlobalMode->GetGlobalDebugMode() || fGlobalMode->IsPedestalMode() || fSaveAnalog){
    if(GetChID()==0) hNoEventsReconstructed->Fill(1);
  }
  //  if(GetChID()>89) return;//Beth 10/3/22: There are only 90 PVeto channels but in EVetoBL.cfg 96 channels are still listed
  if(fUsePulseProcessing==0){ 
    Double_t IsZeroSup = ZSupHit(fZeroSuppression,1000.);  //Beth 10/3/22: we should use the parameter signal window instead of hard-coded 1000. fZeroSuppression is not in EVetoBL.cfg but defaults to 5.
...
void DigitizerChannelEVetoBL::SetAbsSignals(Double_t ped){
  Double_t ScaleFactor=1;
  //chagain defined only for first 90 channels
  if(fChannelEqualisation&&GetChID()<90)  ScaleFactor=SetEVetoBLChaGain();   
  //  std::cout<<"EVetoBL "<<GetChID()<<" ScaleFactor "<<ScaleFactor<<std::endl;
...
void DigitizerChannelEVetoBL::AnalogPlotting(){
  //plot analog signals
...
void DigitizerChannelEVetoBL::HitPlots(std::vector<TRecoVHit *> &hitVec){
...
Double_t DigitizerChannelEVetoBL::SetEVetoBLChaGain(){

  std::ifstream EnergyCalib;
  char fname[100];
//Beth 8/4/22: To make the name of the calibration file clearer, I named the version I use for the digitizer of 2020 signals (which uses the derivative of the signal) "EVetoBL_EnergyCalibration_DerivativeDigitizer2020.txt"
  if(fEnergyCalibrationFile<2)    sprintf(fname,"config/Calibration/EVetoBL_EnergyCalibration_%d.txt", fEnergyCalibrationFile);

  else if(fEnergyCalibrationFile==2)    sprintf(fname,"config/Calibration/EVetoBL_EnergyCalibration_%s.txt","DerivativeDigitizer2020");
  ...
  if (EnergyCalib.is_open()){
	  ...
	  //  std::cout <<"FileRow  "<< i<<" EVetoBL Calibration Constant "<<fEnergyCalibCh[i]<<std::endl; 
	}
  EnergyCalib.close();
  }
  else{ 
  	std::cout<<"No previous data available for EVetoBL, resorting to default calibration constant (1)"<<std::endl;
  }
...
Double_t DigitizerChannelEVetoBL::SetEVetoBLT0(){

  std::ifstream EVetoBLTimeCalib;
  char fname[100];
  //Int_t Calibration=0;

  if(fApplyTimeCalibration==1){
    if(fTimeCalibrationMethod==1){
      sprintf(fname,"config/Calibration/EVetoBL_TimeCalibration_%s.txt","DerivativeDigitizer2020");
      std::ifstream myFile(fname);

      EVetoBLTimeCalib.open(fname);
      if (EVetoBLTimeCalib.is_open()){
	double temp;
	for (int i=0;i<96;i++){
	  EVetoBLTimeCalib >> temp >> fTimeCalibCh[i];
	  //std::cout <<"FileRow  "<< i<<" EVetoBL Calibration Constant "<<fTimeCalibCh[i]<<std::endl; 
	}
	EVetoBLTimeCalib.close();
      }
    }
    else if(fApplyTimeCalibration==1&&fTimeCalibrationMethod==2){
      //Beth: hard coded numbers come from my analysis 26/9/22. They represent the length of the veto cables and any detector effects that contribute to the time measurement. They're found by finding the peak of time differences in EVetoBL-SAC Bremsstrahlung in data, using the run with B field reversed, and subtracting the peak of time differences in EVetoBL-SAC Bremsstrahlung in the full-beamline MC
      if(GetChID()<48) 	fTimeCalibCh[GetChID()]=40.2;
      else fTimeCalibCh[GetChID()]=37.4;
    }
    else{ 
      std::cout<<"No previous data available for EVetoBL, resorting to default calibration constant (0)"<<std::endl;
      for (int i=0;i<96;i++){
	fTimeCalibCh[i]=0;
      }
    ...
    if(fApplyTimeCalibration==1)    std::cout<<"Unknown EVetoBL time calibration file,"<<fname<<" resorting to default calibration constant (0)"<<std::endl;
    for (int i=0;i<96;i++){
      fTimeCalibCh[i]=0;
    }
...
Double_t DigitizerChannelEVetoBL::TailHeightDerivative(Int_t DeltaT){//DeltaT in samples. Returns fraction of maximum signal height that a signal will have at time DeltaT samples after the peak
