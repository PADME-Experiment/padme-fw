...
EVetoBLCalibration::EVetoBLCalibration()
  : PadmeVCalibration()
{
  std::cout<<"EVetoBLCalibration being created ............"<<std::endl ;
...
void EVetoBLCalibration::Init(PadmeVRecoConfig *cfg, RecoVChannelID *chIdMgr ){

  PadmeVCalibration::Init(cfg, chIdMgr );
  
  fCalibrationFile  = (int)cfg->GetParOrDefault("EnergyCalibration", "CalibrationFile", 1); 
  std::cout <<"EVetoBL Calibration File Chosen "<<fCalibrationFile<<std::endl; 
  ...
  for (unsigned int j=0; j<chIds.size();++j)
	{
	  ...
	  //std::cout<<" init calib consts .... j "<<j<<" ID="<<chIds[j]<<" .... READ FROM FILE ... "<<channelGain<<" "<<std::endl;
         ...
         std::cout<<"EVetoBLCalibration::Init calib consts .... j "<<j<<" ID="<<chIds[j]<<" .... READ FROM FILE ... "<<channelGain<<" "<<std::endl;
	  ...
	  else std::cout<<"EVetoBLCalibration::Init - WARNING - no entry found in energy calib map for id="<<chIds[j]<<std::endl;  
	}

}
void EVetoBLCalibration::ReadCalibrationConstants(){
  ...
  sprintf(fname,"config/Calibration/EVetoBL_EnergyCalibration_%d.txt", fCalibrationFile);
  Calib.open(fname);
  if (Calib.is_open()){
  	double temp;
  	for (int i=0;i<96;i++){
          Calib >> temp >> fCalibCh[i];
          //std::cout <<"FileRow  "<< i<<" EVetoBL Calibration Constant "<<fCalibCh[i]<<std::endl; 
	}
  Calib.close();
  }
  else{ 
