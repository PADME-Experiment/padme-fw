...
void DigitizerChannelPVetoBL::PrintConfig(){
  //  std::cout << "Signal width: " << fSignalWidth << " samples" << std::endl;
  //  std::cout << "fUseAbsSignals: " << fUseAbsSignals << std::endl;  
}

DigitizerChannelPVetoBL::~DigitizerChannelPVetoBL(){
  if(fGlobalMode->IsPedestalMode() || fGlobalMode->GetGlobalDebugMode() || fSaveAnalog){
    SaveDebugHistos();
  }
}

void DigitizerChannelPVetoBL::Init(GlobalRecoConfigOptions *gMode, PadmeVRecoConfig *cfg){
  ...
  fTotalAnalogs = cfg->GetParOrDefault("Output","TotalAnalogs",90); //Beth 23/2/22: total number of analog signals to write to PVetoBLRecoAn.root

  fDigiFileOut = cfg->GetParOrDefault("Output","DigiFileOut","PVetoBLRecoAn.root");
...
Double_t DigitizerChannelPVetoBL::CalcPedestal() {  //get NSamples from data card
  fPed = 0.;
  fNPedSamples = 0;
...
Double_t DigitizerChannelPVetoBL::ZSupHit(Float_t Thr, UShort_t NAvg) {// NAvg = 1000, Thr by default is fZeroSuppression = 5. This parameter isn't in the config file, probably it should be?
...
Double_t DigitizerChannelPVetoBL::CalcChaTime(std::vector<TRecoVHit *> &hitVec){//copied and modified from 211019-padmefw, Beth 18/2/22. 22/2/22 Made it independent of pedestal and iMax
  RawGetMax=0;
  DerivGetMax=0;
  ...
  for (UShort_t ii = 0 ; ii != index.size() ; ii++) {
    ...
    Hit = new TRecoVHit();
    //    std::cout<<SetPVetoBLT0()<<std::endl;
    Hit->SetTime(tDerivSortHitVec[ii]-SetPVetoBLT0());//SetPVetoBLT0 gives the time offset of the channel
...
void DigitizerChannelPVetoBL::PrepareDebugHistos(){ //Beth 20/10/21 copied from 190917padme-fw, in turn copied from Mauro's DigitizerChannelECal structure to create a set of debug histograms that are produced only in debug mode
  ...
  hDerivV                     = new TH1F("DerivV","DerivV",800,0,400);
  hDerivVChannels20to70       = new TH1F("DerivVChannels20to70","DerivVChannels20to70",800,0,400);
  hDerivVOneHit               = new TH1F("DerivVOneHit","DerivVOneHit",800,0,400);
  hDerivVOneHitChannels20to70 = new TH1F("DerivVOneHitChannels20to70","DerivVOneHitChannels20to70",800,0,400);
  hDerivVCorrect              = new TH1F("DerivVCorrect","DerivVCorrect",800,0,400);
  hDerivVCorrectChannels20to70= new TH1F("DerivVCorrectChannels20to70","DerivVCorrectChannels20to70",800,0,400);
  hHitTime                    = new TH1F("HitTime","HitTime",400,0,800);
  hHitEnergy                  = new TH1F("HitEnergy","HitEnergy",600,0,6);
  hHitEnergySingleHit         = new TH1F("HitEnergySingleHit","HitEnergySingleHit",600,0,6);
  ...
  hNegativeEnergyCha = new TH1F("hNegativeEnergyCha",";PVetoBLChannelID;",90,0,90);
  ...
  for(int ii=0;ii<96;ii++){
    ...
    hDerivVPerChannel[ii] = new TH1F(name,name,800,0,400);

    sprintf(name, "DerivVOneHitChannel%d",ii);
    hDerivVOneHitPerChannel[ii] = new TH1F(name,name,800,0,400);

    sprintf(name, "DerivVCorrectChannel%d",ii);
    hDerivVCorrectPerChannel[ii] = new TH1F(name,name,800,0,400);
...
void DigitizerChannelPVetoBL::SaveDebugHistos(){
...
void DigitizerChannelPVetoBL::Reconstruct(std::vector<TRecoVHit *> &hitVec){  //using TSpectrum
  if(fGlobalMode->GetGlobalDebugMode() || fGlobalMode->IsPedestalMode() || fSaveAnalog){
    if(GetChID()==0) hNoEventsReconstructed->Fill(1);
  }
  if(GetChID()>89) return;//Beth 10/3/22: There are only 90 PVetoBL channels but in PVetoBL.cfg 96 channels are still listed
  if(fUsePulseProcessing==0){ 
    Double_t IsZeroSup = ZSupHit(fZeroSuppression,1000.);  //Beth 10/3/22: we should use the parameter signal window instead of hard-coded 1000. fZeroSuppression is not in PVetoBL.cfg but defaults to 5.
...
void DigitizerChannelPVetoBL::SetAbsSignals(Double_t ped){
  Double_t ScaleFactor=1;

  if(fChannelEqualisation&&GetChID()<90)  ScaleFactor=SetPVetoBLChaGain();  
  //  std::cout<<"PVetoBL "<<GetChID()<<" ScaleFactor "<<ScaleFactor<<std::endl;
...
void DigitizerChannelPVetoBL::AnalogPlotting(){
  //plot analog signals
...
void DigitizerChannelPVetoBL::HitPlots(std::vector<TRecoVHit *> &hitVec){
  ...
  for(UInt_t myiHit=0;myiHit<fNFoundPerChannel[GetChID()];myiHit++){
    ...
    if(myiHit>1){
      ...
      hDeltaTHitsinCha->Fill(tDerivSortHitVec[myiHit]-tDerivSortHitVec[myiHit-1]);
      if(GetChID()>29&&GetChID()<71)     hDeltaTHitsinChaChas30to70->Fill(tDerivSortHitVec[myiHit]-tDerivSortHitVec[myiHit-1]);
    }
  }
  
}

Double_t DigitizerChannelPVetoBL::SetPVetoBLChaGain(){

  std::ifstream EnergyCalib;
  char fname[100];
//Beth 8/4/22: To make the name of the calibration file clearer, I named the version I use for the digitizer of 2020 signals (which uses the derivative of the signal) "PVetoBL_EnergyCalibration_DerivativeDigitizer2020.txt"
  if(fEnergyCalibrationFile<2)    sprintf(fname,"config/Calibration/PVetoBL_EnergyCalibration_%d.txt", fEnergyCalibrationFile);

  else if(fEnergyCalibrationFile==2)    sprintf(fname,"config/Calibration/PVetoBL_EnergyCalibration_%s.txt","DerivativeDigitizer2020");
  ...
  if (EnergyCalib.is_open()){
	  ...
	  //          std::cout <<"FileRow  "<< i<<" PVetoBL EnergyCalibration Constant "<<fEnergyCalibCh[i]<<std::endl; 
	}
  EnergyCalib.close();
  }
  else{ 
  	std::cout<<"No previous data available for PVetoBL, resorting to default ENERGY calibration constant (1)"<<std::endl;
  }
...
Double_t DigitizerChannelPVetoBL::SetPVetoBLT0(){

  std::ifstream PVetoBLTimeCalib;
  char fname[100];
  //Int_t Calibration=0;

  if(fApplyTimeCalibration==1){
    if(fTimeCalibrationMethod==1){
      sprintf(fname,"config/Calibration/PVetoBL_TimeCalibration_%s.txt","DerivativeDigitizer2020");
      std::ifstream myFile(fname);
    
      PVetoBLTimeCalib.open(fname);
      if (PVetoBLTimeCalib.is_open()){
	double temp;
	for (int i=0;i<96;i++){
	  PVetoBLTimeCalib >> temp >> fTimeCalibCh[i];
	  //std::cout <<"FileRow  "<< i<<" PVetoBL Calibration Constant "<<fTimeCalibCh[i]<<std::endl; 
	}
	PVetoBLTimeCalib.close();
      }
    }
    else if(fApplyTimeCalibration==1&&fTimeCalibrationMethod==2){
      //Beth: hard coded numbers come from my analysis 26/9/22. They represent the length of the veto cables and any detector effects that contribute to the time measurement. They're found by finding the peak of time differences in PVetoBL-SAC Bremsstrahlung in data, and subtracting the peak of time differences in PVetoBL-SAC Bremsstrahlung in the full-beamline MC
      if(GetChID()<48) 	fTimeCalibCh[GetChID()]=42.5;
      else fTimeCalibCh[GetChID()]=39.9;
    }
    else{ 
      std::cout<<"No previous data available for PVetoBL, resorting to default TIME calibration constant (0)"<<std::endl;
      for (int i=0;i<96;i++){
	fTimeCalibCh[i]=0;
      }
    }
    ...
    if(fApplyTimeCalibration==1)    std::cout<<"Unknown PVetoBL time calibration file, resorting to default TIME calibration constant (0)-2"<<std::endl;
    for (int i=0;i<96;i++){
      fTimeCalibCh[i]=0;
    }
...
Double_t DigitizerChannelPVetoBL::TailHeightDerivative(Int_t DeltaT){//DeltaT in samples. Returns fraction of maximum signal height that a signal will have at time DeltaT samples after the peak
