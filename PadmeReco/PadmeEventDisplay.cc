...
//        if(stat(Form(InputFileName.Data()), &filestat) == 0)
    ...
    // Initialize reconstruction and read first event
    PadmeReco = new PadmeReconstruction(&InputFileNameList, ConfFileName, OutputFile, NEvt, Seed);
    PadmeReco->NextEvent();

    TApplication PadmeEventDisplayApp("PadmeEventDisplayApp", &argc, argv);
    gROOT->Reset("a");
    ...
    if(gROOT->IsBatch()) {
      std::cout << "error: PadmeEventDisplay cannot run in batch mode." << std::endl;
      exit(EXIT_FAILURE);
    }

    TEveManager::Create();
    gEve->GetMainWindow()->SetWindowName("Padme Event Display");
    //gEve->GetMainWindow()->MoveResize(0, 0, 1398, 1136);

    gEve->FullRedraw3D(kTRUE);
    PadmeEventDisplayApp.Run();
