...
void DigitizerChannelReco::SaveDebugHistos(){
  ...
  if(fProcessing==0){  
    for(int i= 0; i<96;i++){
      std::cout<<"Ch " <<i<< " AmpSpec " <<AmpSpectraChOverflows[i]<<" AmpCharge "<<AmpChargeSpectraChOverflows[i]<<" AmpCharge2DX "<<AmpCharge2DXChOverflows[i]<<" AmpCharge2DY "<<AmpCharge2DYChOverflows[i]<<" entries "<<hAmpSpectrum[i]->GetEntries()<<std::endl;
    } 
  }
...
void DigitizerChannelReco::HistoFill(){  
  ...
  //  std::cout<<"fPed = "<<fPed<<std::endl;
	...
	if(CalcTime(fIMax)>10.) { //use the time cut
	  ...
	  if(GetChID()<82&&TMath::MaxElement(fNSamples,Signal)>2000) AmpSpectraChOverflows[GetChID()]++;
	  else if(GetChID()>81&&TMath::MaxElement(fNSamples,Signal)>4000) AmpSpectraChOverflows[GetChID()]++;

	  hAmpChargeRatio[GetChID()]->Fill(TMath::MaxElement(fNSamples,Signal)/fCharge);
	  if(fNewPed==0&&TMath::MaxElement(fNSamples,Signal)/fCharge>25000) AmpChargeSpectraChOverflows[GetChID()]++;
	  else if(fNewPed==1&&TMath::MaxElement(fNSamples,Signal)/fCharge>25000) AmpChargeSpectraChOverflows[GetChID()]++;

	  hAmpCharge2D[GetChID()]->Fill(TMath::MaxElement(fNSamples,Signal),fCharge);
	  if(GetChID()<72&&TMath::MaxElement(fNSamples,Signal)>800) AmpCharge2DXChOverflows[GetChID()]++;
	  else if(GetChID()<83&&TMath::MaxElement(fNSamples,Signal)>1500) AmpCharge2DXChOverflows[GetChID()]++;
	  else if(GetChID()>82&&TMath::MaxElement(fNSamples,Signal)>3500) AmpCharge2DXChOverflows[GetChID()]++;

	  if(GetChID()<72&&fCharge>0.2) AmpCharge2DYChOverflows[GetChID()]++;
	  else if(GetChID()<83&&fCharge>0.2) AmpCharge2DYChOverflows[GetChID()]++;
	  else if(GetChID()>82&&fCharge>0.2) AmpCharge2DYChOverflows[GetChID()]++;
