...
int db_process_check(int proc_id)
{
  ...
  sprintf(sqlCode,"SELECT id FROM trigger_process WHERE id = %d",proc_id);
  if ( mysql_query(DBHandle,sqlCode) ) {
...
int db_process_open(int proc_id,time_t t)
{
  ...
  t_st = gmtime(&t);
  //sprintf(sqlCode,"UPDATE trigger_process SET status = 1, time_start = '%04d-%02d-%02d %02d:%02d:%02d' WHERE id = %d",
  //	  1900+t_st->tm_year,t_st->tm_mon+1,t_st->tm_mday,t_st->tm_hour,t_st->tm_min,t_st->tm_sec,proc_id);
  sprintf(sqlCode,"UPDATE trigger_process SET time_start = '%04d-%02d-%02d %02d:%02d:%02d' WHERE id = %d",
	  1900+t_st->tm_year,t_st->tm_mon+1,t_st->tm_mday,t_st->tm_hour,t_st->tm_min,t_st->tm_sec,proc_id);
  if ( mysql_query(DBHandle,sqlCode) ) {
...
int db_process_close(int proc_id,time_t t, unsigned long int size, unsigned int nevt)
{
  ...
  t_st = gmtime(&t);
  //sprintf(sqlCode,"UPDATE trigger_process SET status = 2, time_stop = '%04d-%02d-%02d %02d:%02d:%02d', total_events = %u, total_size = %lu WHERE id = %d",
  //	  1900+t_st->tm_year,t_st->tm_mon+1,t_st->tm_mday,t_st->tm_hour,t_st->tm_min,t_st->tm_sec,nevt,size,proc_id);
  sprintf(sqlCode,"UPDATE trigger_process SET time_stop = '%04d-%02d-%02d %02d:%02d:%02d', total_events = %u, total_size = %lu WHERE id = %d",
	  1900+t_st->tm_year,t_st->tm_mon+1,t_st->tm_mday,t_st->tm_hour,t_st->tm_min,t_st->tm_sec,nevt,size,proc_id);
  if ( mysql_query(DBHandle,sqlCode) ) {
...
int db_process_get_status(int proc_id)
{
  ...
  sprintf(sqlCode,"SELECT status FROM trigger_process WHERE id = %d",proc_id);
  if ( mysql_query(DBHandle,sqlCode) ) {
    printf("DB::db_run_check - ERROR executing SQL query: %s\n%s\n", mysql_error(DBHandle),sqlCode);
    return DB_SQLERROR;
  }
...
int db_process_set_status(int proc_id,int status)
{
  ...
  sprintf(sqlCode,"UPDATE trigger_process SET status = %d WHERE id = %d",status,proc_id);
  if ( mysql_query(DBHandle,sqlCode) ) {
