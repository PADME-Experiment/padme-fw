  ...
  while(1){
	...
	*/
	...
	if (totalWriteEvents == 0) {
	  ...
	  float dt_ms = dt/80.0E3; // Trigger clock is 80.0MHz: get time interval in ms
	  ...
	  printf("- Trigger %9u %#018lx %13lu %#04x %4u %1x %1x %11.3fms %7d.%3.3dms\n",totalWriteEvents,word,trig_time,trig_map,trig_count,trig_fifo,trig_auto,dt_ms,dclock_ms,dclock_us);
	}
	old_time = trig_time;
