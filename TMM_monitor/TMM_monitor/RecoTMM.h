...
public :
   ...
   RecoTMM(TObjArray *inputFileNameList, int RunID=0, int DetRunID=0, int maxEvents=0, TString outputFileName="Monitor_TMM.root");   virtual ~RecoTMM();
   ...
   float APV1_fqx = 0.612537*0.879458;
   float APV2_fqx = 1;
   float APV3_fqx = 0.434602*0.949184;

   float APV1_fqy = 0.658494*0.946133;
   float APV2_fqy = 1;
   float APV3_fqy = 1.08688*0.978012;
   ...
   // vector of non-working physics strips:
   vector<int> BrokenStrips;

   // blocks of events for beam spot stability monitoring
   // int NevtBlock = 1000;
   vector<double> x_block[TMMCH_N_Readout];
   vector<double> q_block[TMMCH_N_Readout];
   ...
   // TH2F *htimestrip[TMMCH_N_Readout] = {0}; //time vs xstrip distribution
   // TH2F *htimestripFull[TMMCH_N_Readout] = {0}; //time vs xstrip distribution
   // TH2F *hBeamSpotXY = 0; // event-by-event beam spot Y vs X
   // TH2F *hBeamXY = 0;      // charge-weighted X-Y beam map from strip hits
   // TH2F *hqmaxtime[TMMCH_N_Readout] = {0}; //qmax vs time 
   // TH1F *hmaxtime[TMMCH_N_Readout] = {0}; //time of qmax distribution 
   // TH1F *hmaxq[TMMCH_N_Readout] = {0}; //qmax distribution
   ...
   // vector<TH2F*> hBlocktimestrip[TMMCH_N_Readout]; //time vs xstrip distribution
   // vector<TH2F*> hBlocktimestripFull[TMMCH_N_Readout]; //time vs xstrip distribution
   vector<TH1F*> hBlockBeam[TMMCH_N_Readout]; //xstrip distribution charge weighted (selected strips)
   vector<TH1F*> hBlockBeamFull[TMMCH_N_Readout]; //xstrip distribution charge weighted
   // vector<TH1F*> hBlockmaxtime[TMMCH_N_Readout]; //time of qmax distribution 
   // vector<TH1F*> hBlockqmaxtime[TMMCH_N_Readout]; //qmax vs time 
   vector<TH1F*> hBlockmaxq[TMMCH_N_Readout]; //qmax distribution
   // vector<TH2F*> hBlockBeamSpotXY; // block beam spot Y vs X
   // vector<TH2F*> hBlockBeamXY;      // block charge-weighted X-Y beam map from strip hits
   ...
   // TGraphErrors *g_BeamMaxTime[TMMCH_N_Readout] = {0}; // Time of qmax over events 
   ...
   // TGraphErrors *g_BlockBeamMaxTime[TMMCH_N_Readout] = {0}; // Time of qmax over blocks of events for stability monitoring
   ...
   outputFileName(outputFileName)
{
   ...
   for (Int_t iFile = 0; iFile < inputFileNameList->GetEntries(); iFile++) {
      ...
      //    cout << "Reached requested maxEvents while building chain: "
      //         << maxEvents << endl;
      //    break;
      // }
   }
