...
void ReorderStreams(std::vector<double>& vec, int S) {
    int N = vec.size();
    int events_per_stream = N / S;
    std::vector<double> tmp(N);

    for(int old_idx = 0; old_idx < N; old_idx++){
        int stream = old_idx / events_per_stream;
        int pos = old_idx % events_per_stream;
        int new_idx = pos * S + stream;
        tmp[new_idx] = vec[old_idx];
    }
    vec = std::move(tmp);
}
...
bool TargetChargePosition(int runNumber, const string &filePathPattern, RunEventCharge &RunEvQ){
    ...
    // --- 📌 Reorder data due to 5-stream event writing
    const int S = 5; // number of DAQ streams
    if (QLG.size() > S) {
        ReorderStreams(QLG, S);
        ReorderStreams(QLG_err, S);

        ReorderStreams(Qratio_raw, S);
        ReorderStreams(Qratio_raw_err, S);
        ReorderStreams(Qratio_cal, S);
        ReorderStreams(Qratio_cal_err, S);

        // For arrays<3>, reorder block-wise:
        auto reorder3 = [&](vector<array<double,3>>& v) {
            int N = v.size();
            int events_per_stream = N / S;
            vector<array<double,3>> tmp(N);
            for (int old_idx = 0; old_idx < N; old_idx++) {
                int stream = old_idx / events_per_stream;
                int pos = old_idx % events_per_stream;
                int new_idx = pos * S + stream;
                tmp[new_idx] = v[old_idx];
            }
            v = std::move(tmp);
        };

        reorder3(X_qTar_raw);
        reorder3(err_X_qTar_raw);
        reorder3(X_qTar);
        reorder3(err_X_qTar);

        reorder3(coeffX_raw);
        reorder3(coeffX_raw_err);
        reorder3(coeffX);
        reorder3(coeffX_err);

        // Instead, reorder manually:
        auto reorderGOF = [&](vector<GoodnessOfFit>& v){
            int N = v.size();
            int events_per_stream = N / S;
            vector<GoodnessOfFit> tmp(N);
            for (int old_idx = 0; old_idx < N; old_idx++) {
                int stream = old_idx / events_per_stream;
                int pos = old_idx % events_per_stream;
                int new_idx = pos * S + stream;
        ...
        cout << " Reordering applied for 5-stream DAQ storage" << endl;
    }
...
int main(int argc, char** argv) {
    ...
    int runNumber = 80166; // example run number

    string calibFile = "/home/mancinima/LeadGlassCalib2024/padme-fw/TargetRun4/CalibTarget/outputCalibration/TargetCalibrationConst.txt";
    bool calibLoaded = LoadCalibrationConstants(calibFile);
    if (!calibLoaded) {
        cerr << "Calibration constants missing — continuing with unity gains!" << endl;
    }
    
    string rootPattern = "/home/mancinima/LeadGlassCalib2024/padme-fw/TargetRun4/outputReco4/Reco_run_00%d.root";

    // defining the output txt file
    ofstream OutputFit(Form("/home/mancinima/LeadGlassCalib2024/padme-fw/TargetRun4/output/TargetChargePosition_run_00%i.txt", runNumber));
    OutputFit << "==== Calibration LogFile ChCalib ====" << endl;
    // defining the output root file
    TString outputFileName = "outputCalibration/ChCalib_XY_run_00" + to_string(runNumber) + ".root";
    ...
    if (Nfits == 0) {
        ...
        // Create ROOT graphs
        TGraphErrors *gXTar = new TGraphErrors(Nfits, idx.data(), beamPosX_mm.data());
        TGraphErrors *gXTar_raw = new TGraphErrors(Nfits, idx.data(), beamPosX_mm_raw.data());
        ...
        TGraphAttribute(gXTar, "gXTar", "Fit Index", "X [mm]", 20, kRed);
        TGraphAttribute(gXTar_raw, "gXTar_raw", "Fit Index", "X Raw [mm]", 20, kRed);
        // TGraphAttribute(gYTar, "gYTar", "Fit Index", "Y [mm]", 20, kRed);
        TGraphAttribute(gQLG, "gQLG", "Fit Index", "LG Q [pC]", 20, kRed);
        TGraphAttribute(gQratio_raw, "gQratio_raw", "Fit Index", "Q Ratio Raw", 20, kRed);
        TGraphAttribute(gQratio_cal, "gQratio_cal", "Fit Index", "Q Ratio Cal", 20, kRed);
        TGraphAttribute(gsigmaXtar, "gsigmaXtar", "Fit Index", "Sigma X [mm]", 20, kRed);
        TGraphAttribute(gsigmaXtar_raw, "gsigmaXtar_raw", "Fit Index", "Sigma X Raw [mm]", 20, kRed);
        // TGraphAttribute(gsigmaYtar, "gsigmaYYtar", "Fit Index", "Sigma Y [mm]", 20, kRed);
        TGraphAttribute(gChi2_X, "gChi2_X", "Fit Index", "Chi2", 20, kRed);
        TGraphAttribute(gChi2_X_raw, "gChi2_X_raw", "Fit Index", "Chi2 Raw", 20, kRed);
        // TGraphAttribute(gChi2_Y, "gChi2_Y", "Fit Index", "Chi2", 20, kRed);
        TGraphAttribute(gChargeX, "gChargeX", "Fit Index", "QX [pC]", 20, kRed);
        TGraphAttribute(gChargeX_raw, "gChargeX_raw", "Fit Index", "QX Raw [pC]", 20, kRed);
        // TGraphAttribute(gChargeY, "gChargeY", "Fit Index", "QY [pC]", 20, kRed);
