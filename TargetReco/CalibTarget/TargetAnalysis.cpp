...
bool TargetChargePosition(int runNumber, const string &filePathPattern, RunEventCharge &RunEvQ){
    ...
    t->SetBranchAddress("AbsTimeEventUNIX", &AbsTimeEventUNIX);
    ...
    for(Long64_t ev=0; ev<nentries; ev++){
        ...
        if ((int)cur_QLG.size() == BLOCKSIZE) {
            ...
            if (chunk % NCHUNKS == 0) {
                ...
                vUnixTime.push_back(AbsTimeEventUNIX); //verificare che riempa bene il vettore col tempo
...
int main(int argc, char** argv) {
    ...
    double sqrts = sqrt(2 * ebeam * 0.511); // sqrt(2 * E_beam * m_electron)
    if (*end2 != '\0') {
        std::cerr << "Error: ebeam must be a number. Got: " << argv[2] << "\n";
        return 1;
    }

    std::cout << "runNumber=" << runNumber << " ebeam=" << ebeam << " sqrts=" << sqrts << "\n";
    ...
    string rootPattern = "/home/mancinima/LeadGlassCalib2024/padme-fw/TargetRun4/outputReco_NewRun4/Reco_run_00%d.root";
    ...
    ofstream OutputTxt(Form("/home/mancinima/LeadGlassCalib2024/padme-fw/TargetRun4/CalibTarget/outputCalibration/Run4Monitor/Long_run_00%i_.txt", runNumber));
    ...
    // ofstream OutputTxt(Form("/home/mancinima/LeadGlassCalib2024/padme-fw/TargetRun4/CalibTarget/outputCalibration/Run4Monitor/TargetAnalysis_run_00%i_NewReco_100files.txt", runNumber));
    // ofstream DBtxt(Form("/home/mancinima/LeadGlassCalib2024/padme-fw/TargetRun4/CalibTarget/outputCalibration/Run4Monitor/Database/TargetSimple_run_00%i.txt", runNumber));
    ofstream DBtxt(Form("/home/mancinima/LeadGlassCalib2024/padme-fw/TargetRun4/CalibTarget/outputCalibration/Run4Monitor/Database/INUTILE_run_00%i.txt", runNumber));
    DBtxt << "#RunNumber = " << runNumber << " - EBeam = " << ebeam << " - SqrtS = " << sqrts << endl;
    
    // DBtxt << "UnixTime," <<
    //          "q_LG,err_q_LG," <<
    //          "qX_Target,err_qX_Target,qY_Target,err_qY_Target," <<
    //          "Fitted_qX_Target,err_Fitted_qX_Target,Fitted_qY_Target,err_Fitted_qY_Target," <<
    //          "Ratio_qX_Target_LG,err_Ratio_qX_Target_LG,Ratio_qY_Target_LG,err_Ratio_qY_Target_LG," <<
    //          "Ratio_Fitted_qX_Target_LG,err_Ratio_Fitted_qX_Target_LG,Ratio_Fitted_qY_Target_LG,err_Ratio_Fitted_qY_Target_LG," <<
    //          "Ratio_qX_Target_LG_EnCorr,err_Ratio_qX_Target_LG_EnCorr,Ratio_qY_Target_LG_EnCorr,err_Ratio_qY_Target_LG_EnCorr," <<
    //          "Ratio_Fitted_qX_Target_LG_EnCorr,err_Ratio_Fitted_qX_Target_LG_EnCorr,Ratio_Fitted_qY_Target_LG_EnCorr,err_Ratio_Fitted_qY_Target_LG_EnCorr," <<
    //          "X_tar,err_X_Tar,Y_tar,err_Y_Tar," <<
    //          "Fitted_X_tar,err_Fitted_X_Tar,Fitted_Y_tar,err_Fitted_Y_Tar," <<
    //          "sigmaX_Tar,err_sigmaX_Tar" << endl;

    DBtxt << "#period," <<
             "qx,err_qx,qy,err_qy," <<
             "x,err_x,y,err_y," <<
             "sigmax,err_sigmax" << endl;
    
    // defining the output root file
    TString outputFileName = "outputCalibration/Run4Monitor/Long_run_00" + to_string(runNumber) + ".root";
    ...
    // cout << "length evt: " << evt.size() << endl;
    // cout << "length UnixTime: " << UnixTime.size() << endl;
    // cout << "length cX: " << cX.size() << endl;
    // cout << "length Xpos_wmean_raw: " << Xpos_wmean_raw.size() << endl;
    // cout << "length LGQ: " << LGQ.size() << endl;
    ...
    if (Nfits == 0 || cYraw.size() != Nfits) {
        ...
        for(int i=0;i<Nfits;i++){
            ...
            // writing to the output txt file for each fit - Database 
            // cout << "UnixTime = " << std::fixed << std::setprecision(0) << UnixTime[i] << endl;
           
            // DBtxt << std::fixed << std::setprecision(0) << UnixTime[i] << ","
            //       << std::setprecision(4)
            //       << LGQ[i]  << "," << err_LGQ[i] << ","
            //       << QXTot_target_cal[i] << "," << QXTot_target_cal_err[i] << ","
            //       << QYTot_target_raw[i] << "," << QYTot_target_raw_err[i] << ","
            //       << A_X[i] << "," << A_X_err[i] << ","
            //       << A_Y_raw[i] << "," << A_Y_raw_err[i] << ","
            //       << QXratio_cal[i] << "," << QXratio_cal_err[i] << ","
            //       << QYratio_raw[i] << "," << QYratio_raw_err[i] << ","
            //       << gChargeXratio_cal->GetY()[i] << "," << gChargeXratio_cal->GetErrorY(i) << ","
            //       << gChargeYratio_raw->GetY()[i] << "," << gChargeYratio_raw->GetErrorY(i) << ","
            //       << gQXratio_cal_ECorr->GetY()[i] << "," << gQXratio_cal_ECorr->GetErrorY(i) << ","
            //       << gQYratio_raw_ECorr->GetY()[i] << "," << gQYratio_raw_ECorr->GetErrorY(i) << ","
            //       << gChargeXratio_cal_ECorr->GetY()[i] << "," << gChargeXratio_cal_ECorr->GetErrorY(i) << ","
            //       << gChargeYratio_raw_ECorr->GetY()[i] << "," << gChargeYratio_raw_ECorr->GetErrorY(i) << ","
            //       << beamPosX_wm_cal[i] << "," << beamPosX_wm_cal_err[i] << ","
            //       << beamPosY_wm_raw[i] << "," << beamPosY_wm_raw_err[i] << ","
            //       << beamPosX_mm[i] << "," << beamPosX_mm_err[i] << ","
            //       << beamPosY_mm_raw[i] << "," << beamPosY_mm_raw_err[i] << ","
            //       << beamWidthX[i] << "," << beamWidthX_err[i] << endl;
