...
EXECUTABLE = "./RecoTarget.exe"
LIST_DIR   = "ListDir"
OUTPUT_DIR = "./outputReco"


def get_list_file(runid):
    """Return the list file path for a given run."""
    return os.path.join(LIST_DIR, f"run_00{runid}.list")
...
def main():
    ...
    run_input = input(
        "Enter RUN NUMBER(s) (nnnnn) separated by spaces: "
    ).strip()
    ...
    try:
        n_cores = int(
            input("How many CPU cores to use? ").strip()
        )
    ...
    use_nohup = input(
        "Launch in background (nohup)? (yes/no): "
    ...
    print(
        ...
        f"{total_jobs} runs, "
        f"max {n_cores} concurrent jobs.\n"
    )

    while queue or running:

        # Launch jobs while slots are available
        while queue and len(running) < n_cores:

            runid = queue.pop(0)

            core = (
                cores[len(running) % n_cores]
                if taskset_ok
                else None
            )
    ...
    print(
        f"{GREEN}SUCCESS: {len(ok)}{RESET}"
    )

    print(
        f"{RED}FAILED : {len(fail)}{RESET}"
    )
