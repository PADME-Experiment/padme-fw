...
int main(int argc, char* argv[]) {
  ...
  for (Long64_t iev = 0; iev < runNEntries; ++iev) {
    ...
    for(UChar_t brd = 0; brd<rawEv->GetNADCBoards(); brd++){
      ...
      if (!brdID){
        fprintf(stderr,"ERROR - Board ID is 0\n");
        continue;
      }
      ...
      for(UChar_t ch=0; ch<nChn; ch++){
        // TGraphErrors WaveGraphOld(NAVG-1);
        // TGraphErrors WaveGraph1Wf(NAVG-1);
        // TGraphErrors WaveGraphCum(NAVG-1);
        ...
        for(UShort_t s=0; s<NAVG; s++){
          ...
          // fillTGraph(&WaveGraphOld, s, s, AbsRecoSampleOld[s], 0, 0);
          // fillTGraph(&WaveGraph1Wf, s, s, AbsRecoSample1Wf[chan][s], 0, 0);
        ...
        // histoFile->cd();
        // if(nsum>0 && nsum%NWFcumulative==0){ // save 1 over 10 waveforms + cumulative only
          // WaveGraphOld.SetName(Form("WaveGraphOld%i_%i_%i", (int)iev, (int)brdID, (int)chan));
          // WaveGraph1Wf.SetName(Form("WaveGraph1Wf%i_%i_%i", (int)iev, (int)brdID, (int)chan));
          // WaveGraphCum.SetName(Form("WaveGraphCum%i_%i_%i", (int)iev, (int)brdID, (int)chan));
          
          // WaveGraphOld.SetLineWidth(1); WaveGraphOld.SetMarkerStyle(20); WaveGraphOld.SetMarkerSize(.2); WaveGraphOld.SetMarkerColor(kBlue); 
          // WaveGraph1Wf.SetLineWidth(1); WaveGraph1Wf.SetMarkerStyle(20); WaveGraph1Wf.SetMarkerSize(.2); WaveGraph1Wf.SetMarkerColor(kBlue); 
          // WaveGraphCum.SetLineWidth(1); WaveGraphCum.SetMarkerStyle(20); WaveGraphCum.SetMarkerSize(.2); WaveGraphCum.SetMarkerColor(kBlue);
