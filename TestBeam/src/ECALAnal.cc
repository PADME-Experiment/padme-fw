...
void ECALAnal::AnalyzeCharge()
{
  ...
  for(UChar_t b=0;b<nBoards;b++){
	...
	fQChannel[bid][cnr] += -fSampleReco[s]/50*1E-9/1E-12; // dT(bin)=1ns, R=50 Ohm, Q in pC
      ...
      fECALHisto->Fill1DHisto(Form("hPedCalo%d",cnr),Avg);
      fECALHisto->Fill1DHisto(Form("hQCh%d",cnr),fQChannel[bid][cnr]);
    ...
    fECALHisto->Fill1DHisto("hQTotCalo",fQTotal[bid]);
...
void ECALAnal::AnalyzePosition()
{
  ...
  for(UChar_t b=0;b<nBoards;b++){
    ...
    fECALHisto->Fill2DHisto("hPosCalo",XcryTot/fQTotal[bid],YcryTot/fQTotal[bid]);
