...
void ToyBBY_bhabha(double EBeam, int nEvent, array<TVector3,3> runconfig, double AngBeamDirX, double AngBeamDirY, const char* RootInput, const char* outputRoot)
{  
...
tFull->SetBranchAddress("FFinalP1_positron", &FFinalP1_ele); // electron is positron if BABAYAGA EVENTS (and viceversa)
tFull->SetBranchAddress("FFinalP2_positron", &FFinalP2_ele); // electron is electron if CALCHEP EVENTS (and positron is positron obv)
tFull->SetBranchAddress("FFinalP3_positron", &FFinalP3_ele);
tFull->SetBranchAddress("FFinalP1_electron", &FFinalP1_pos);
tFull->SetBranchAddress("FFinalP2_electron", &FFinalP2_pos);
tFull->SetBranchAddress("FFinalP3_electron", &FFinalP3_pos);
...
int main(int argc, char** argv) {
    ...
    cout << "##############Running ToyBBY_bhabha with parameters ##############" << endl;
    cout << "EBeam=" << EBeam << " MeV  nEvent=" << nEvent
         << " run=" << run << " thetaX=" << ThetaX << " thetaY=" << ThetaY << "\n"
         << "Input : " << inputRootFile << "\n"
         << "Output: " << outputRootFile << endl;

    ToyBBY_bhabha(EBeam, nEvent, runconfig, ThetaX, ThetaY, inputRootFile, outputRootFile);
