...
void ToyCalc_Sig(double EBeam, int nEvent, array<TVector3,3> runconfig, double AngBeamDirX, double AngBeamDirY, const char* RootInput, const char* outputRoot)
{  
...
int main(int argc, char** argv) {
    ...
    cout << "############## Running ToyCalc_Sig with parameters ##############" << endl;
    cout << "EBeam=" << EBeam << " MeV  nEvent=" << nEvent
         << " run=" << run << " thetaX=" << ThetaX << " thetaY=" << ThetaY << "\n"
         << "Input : " << inputRootFile << "\n"
         << "Output: " << outputRootFile << endl;

    ToyCalc_Sig(EBeam, nEvent, runconfig, ThetaX, ThetaY, inputRootFile, outputRootFile);
