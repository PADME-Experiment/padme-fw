...
public:
  ...
  Int_t GetNHitsPerAPV(int apv, int bdid){if (apv>=0 && apv <2 && bdid >=0 && bdid < 16) {return fNHitsPerAPV[apv][bdid];} else {return -1;}}
private:
  Bool_t InitHistos();
  Int_t fNHitsPerAPV[2][16];//number of hits per apv,board
