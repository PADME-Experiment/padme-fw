...
Bool_t ECalCalib22::Init(Bool_t fHistoModeVal, TString InputHistofileVal)
{
  ...
  InFile.close();
  if (iChNum != 613)
    cout << "**** " << (616 - iChNum) << " ECal Channels missing in ECalEnergyCalibration_6.dat **** " << endl;
...
Bool_t ECalCalib22::InitHistos()
{
  ...
  fHS->BookHistoList("ECalCalib22", "CREnCH-1", 220, -10, 100);
  fHS->BookHistoList("ECalCalib22", "CREnCH-1_Vert", 220, -10, 100);
...
Bool_t ECalCalib22::ChannelLandauFit()
{
  ...
  ofstream OutFile;
  if (NewCalib)
    OutFile.open(Form("%sECalEnergyCalibration_8.dat", fcfgPath.Data()));
  ofstream ErrFile(Form("%s_%s.txt", fcfgPath.Data(), fNRun.Data()));
  cout << "Performing Landau fits" << endl
       << endl;
  ...
  if (ErrFile.is_open())
  {
    ...
    for (iCh = 0; iCh < NTOTCh; iCh++)
    { // Fitting each histo with a Laundau function
      ...
      if (NonEmpty > 10)
      {
        ...
        if (Chi2 > 2. || Chi2 < 0.2)
        {
          ...
          if (Cross[iCh] == 0)
            ...
            ECalChEff[iCh] = (Double_t)MiddleCross[iCh] / (Double_t)Cross[iCh];
        ...
        if (NewCalib && OutFile.is_open())
          OutFile << " " << ECalChNum[iCh] / 100 << " " << ECalChNum[iCh] % 100 << " " << FBdId[iCh] << " " << FChId[iCh] << " " << ECalChMVP[iCh] * 15. << endl;
        ...
        if (ECalChNum[iCh] != 1727 && ECalChNum[iCh] != 2407)
          fHS->FillHisto2List("ECalCalib22", "CalibRatio", ECalChNum[iCh] / 100, ECalChNum[iCh] % 100, ECalChMVP[iCh] * 15. / OldCalib[iCh]);
    ...
    cout << endl;
    if (NewCalib)
      cout << "\t\t---> Calibration constants can be found in ECalEnergyCalibration_8.dat" << endl
           << endl;
    cout << "\t\t---> Calibration Errors log are in BadUnits_" << fNRun.Data() << ".txt, plots can be found in BadCalib_" << fNRun.Data() << ".root" << endl
         << endl;
  }
  else
  {
