  ...
  for(Int_t iV = 0; iV < fEvent->MCTruthEvent->GetNVertices(); iV++) {  
    TMCVertex *mcVtx = fEvent->MCTruthEvent->Vertex(iV);
      if(mcVtx->GetProcess() =="Bhabha"){
        ...
        if(fFillLocalHistograms) fhSvcVal->FillHistoList("ECalSel", "EnSignalBhabha",pcleE, 1.);
        } 
    }
    return true;
}
...
Int_t ECalSel::OneClusTagAndProbeSel(){
  ...
  for (int h1=0; h1< fECal_clEvent->GetNElements(); ++h1) {    
    ...
    if(!((cluEnergy[0]-pg)> tpLow && (cluEnergy[0]-pg)<tpHigh)) continue; 
    ...
    for (h2=0; h2< fECal_clEvent->GetNElements(); ++h2) {
      ...
      fhSvcVal->FillHisto2List("ECalSel",Form("ECal_TP_DTHEVsDPHIAbs_probe_nocut"), 
        ...
        if ((DeltaPhiAbs>fMeanDPhi-fSigmaCut*fSigmaDPhi && DeltaPhiAbs<fMeanDPhi+fSigmaCut*fSigmaDPhi) && (DeltaTheta>fMeanDTheta-fSigmaCut*fSigmaDTheta && DeltaTheta<fMeanDTheta+fSigmaCut*fSigmaDTheta)){
...
Int_t ECalSel::TwoClusSel(){
  ...
  for (int h1=0; h1< fECal_clEvent->GetNElements(); ++h1) {
    tempClu[0] = fECal_clEvent->Element((int)h1);
    if (fFillLocalHistograms) fhSvcVal->FillHisto2List("ECalSel",Form("ECal_SC_yvsx_EweightAll"), tempClu[0]->GetPosition().X(), tempClu[0]->GetPosition().Y(), tempClu[0]->GetEnergy());
    if(fFillCalibHistograms) fhSvcVal->FillHisto2List("ECalSel",Form("ECal_SC_yvsxAll"), tempClu[0]->GetPosition().X(), tempClu[0]->GetPosition().Y(), 1.);
  ...
  for (int h1=0; h1< fECal_clEvent->GetNElements(); ++h1) {
    ...
    for (int h2=h1+1; h2< fECal_clEvent->GetNElements(); ++h2) { //corretto partire da h1+1?
      ...
      if (fabs(dt) < fMaxTimeDistance && dr > fMinGGDistance) { 
          ...
          if (fFillLocalHistograms) fhSvcVal->FillHistoList("ECalSel","ECal_SC_Chi2",ChiEnergy , 1.);
          if (nPaired == 0){ 
            isPaired = h2;
            tempChiEn = ChiEnergy;
          } else if(nPaired > 0){
    ...
    if (fFillLocalHistograms) fhSvcVal->FillHisto2List("ECalSel","ECal_SC_Chi2vsNPaired" , nPaired,tempChiEn, 1.);
  ...
  //if (!(elliEnergy < 4 && TMath::Abs(TMath::Pi()-dphi)/sigmadphi < 3)) continue;
	for (int kk=0; kk<2; kk++){
	  double seedEnergy = fECal_hitEvent->Hit(tempClu[kk]->GetSeed())->GetEnergy();
	   ...
	   if (fFillLocalHistograms) fhSvcVal->FillHisto2List("ECalSel",Form("ECal_SC_NHits_vs_Eexp"), pg[kk], tempClu[kk]->GetNHitsInClus(), 1.);
  ...
  if(!(DeltaPhiAbs>fMeanDPhi-fSigmaCut*fSigmaDPhi && DeltaPhiAbs<fMeanDPhi+fSigmaCut*fSigmaDPhi))  continue;
  if(!(DeltaTheta>fMeanDTheta-fSigmaCut*fSigmaDTheta && DeltaTheta<fMeanDTheta+fSigmaCut*fSigmaDTheta))  continue;
    ...
    if(fEvent->RecoEvent->GetEventStatusBit(TRECOEVENT_STATUSBIT_SIMULATED)){
              if(fMCTruthECal->GetVtxFromCluID((int)isPaired)<0){
                ...
                fhSvcVal->FillHisto2List("ECalSelMCTruth",Form("ECal_SC_DCOGYvsDCOGY_probe_%s",processSelected.Data()), cog.X()-fGeneralInfo->GetCOG().X(), cog.Y()-fGeneralInfo->GetCOG().Y(),1.);
				        fhSvcVal->FillHistoList("ECalSelMCTruth",Form("ECal_SC_E1plusE2_%s",processSelected.Data()), cluEnergy[0]+ cluEnergy[1]);
  ...
  fhSvcVal->FillHisto2List("ECalSel","ECal_SC_DE_vs_EClu2_cut", cluEnergy[1], cluEnergy[1]-pg[1],1 );
	...
	for (int kk = 0; kk < 2; kk++){
	  if(fFillLocalHistograms)fhSvcVal->FillHisto2List("ECalSel",Form("ECal_E_Radius"), cluPosRel[kk].Perp(), tempClu[kk]->GetEnergy());
	  if(fFillLocalHistograms)fhSvcVal->FillHistoList("ECalSel",Form("ECal_RadiusECut"),
				  (cluPosRel[kk].Perp()-fGeneralInfo->GetRadiusMin())/(fGeneralInfo->GetRadiusMax()-fGeneralInfo->GetRadiusMin()));
	  if(fFillLocalHistograms)fhSvcVal->FillHistoList("ECalSel",Form("ECal_ERCut"),
	  ...
	  if(fFillLocalHistograms)fhSvcVal->FillHisto2List("ECalScale",hname.Data(),pg[kk],cluEnergy[kk]/pg[kk]-1,1.); 
	...
	for (int ith = 0; ith<fNThetaBins; ith++){ // loop on theta
	  ...
	  for (int iph = 0; iph<fNPhiDirBins; iph++){ // loop on phi
	    ...
	    if(fFillLocalHistograms) fhSvcVal->FillHisto2List("ECalSel","DPhiVsRotVsRunCos",thetaRot,phiRot,1.+cosRel); 
	    if (cosRel < -0.998) {
	      if(fFillLocalHistograms)fhSvcVal->FillHisto2List("ECalSel","DPhiVsRotVsRun",thetaRot,phiRot,1.); 
	    }
	  }
	}
...
Bool_t ECalSel::InitHistos()
{
  ...
  // fhSvcVal->BookHisto2List("ECalSel","ECal_TP_DEVsE_NOcut_tag_novtx", 100, 100,400, 800, -400, 400.);
  // fhSvcVal->BookHisto2List("ECalSel","ECal_TP_DEVsE_NOcut_tag_eIoni", 100, 100,400, 800, -400, 400.);
  // fhSvcVal->BookHisto2List("ECalSel","ECal_TP_DEVsE_NOcut_tag_eBrem", 100, 100,400, 800, -400, 400.);
  // fhSvcVal->BookHisto2List("ECalSel","ECal_TP_DEVsE_NOcut_tag_annihil", 100, 100,400, 800, -400, 400.);
  // fhSvcVal->BookHisto2List("ECalSel","ECal_TP_DEVsE_NOcut_tag_Bhabha", 100, 100,400, 800, -400, 400.);
  ...
  // fhSvcVal->BookHistoList("ECalSel","ECal_TP_DE_Probe_eIoni", 800, 0, 400);
  // fhSvcVal->BookHistoList("ECalSel","ECal_TP_DE_Probe_eBrem", 800, 0, 400);
  // fhSvcVal->BookHistoList("ECalSel","ECal_TP_DE_Probe_annihil", 800, 0, 400);
  // fhSvcVal->BookHistoList("ECalSel","ECal_TP_DE_Probe_Bhabha", 800, 0, 400);
  ...
  // fhSvcVal->BookHistoList("ECalSel","ECal_TP_DPHIAbs_probe_eIoni", 1200, 0.,4*TMath::Pi());
  // fhSvcVal->BookHistoList("ECalSel","ECal_TP_DPHIAbs_probe_eBrem", 1200, 0.,4*TMath::Pi());
  // fhSvcVal->BookHistoList("ECalSel","ECal_TP_DPHIAbs_probe_annihil", 1200, 0.,4*TMath::Pi());
  // fhSvcVal->BookHistoList("ECalSel","ECal_TP_DPHIAbs_probe_Bhabha", 1200, 0.,4*TMath::Pi());
  ...
  // fhSvcVal->BookHisto2List("ECalSel","ECal_TP_DEVsE_cut_probe_eIoni", 100, 100,400, 800, -400, 400.);
  // fhSvcVal->BookHisto2List("ECalSel","ECal_TP_DEVsE_cut_probe_annihil", 100, 100,400, 800, -400, 400.);
  // fhSvcVal->BookHisto2List("ECalSel","ECal_TP_DEVsE_cut_probe_eBrem", 100, 100,400, 800, -400, 400.);
  // fhSvcVal->BookHisto2List("ECalSel","ECal_TP_DEVsE_cut_probe_Bhabha", 100, 100,400, 800, -400, 400.);
  ...
  // fhSvcVal->BookHisto2List("ECalSel",Form("ECal_SC_Theta1VsTheta2_eIoni"), 900, -TMath::Pi(),TMath::Pi(), 900, -TMath::Pi(),TMath::Pi());
  // fhSvcVal->BookHisto2List("ECalSel",Form("ECal_SC_Theta1VsTheta2_eBrem"), 900, -TMath::Pi(),TMath::Pi(), 900, -TMath::Pi(),TMath::Pi());
  // fhSvcVal->BookHisto2List("ECalSel",Form("ECal_SC_Theta1VsTheta2_annihil"), 900, -TMath::Pi(),TMath::Pi(), 900, -TMath::Pi(),TMath::Pi());
  // fhSvcVal->BookHisto2List("ECalSel",Form("ECal_SC_Theta1VsTheta2_Bhabha"), 900, -TMath::Pi(),TMath::Pi(), 900, -TMath::Pi(),TMath::Pi());
  fhSvcVal->BookHisto2List("ECalSel",Form("ECal_SC_ThetaSumVsDeltaTheta"), 900,0 , 3*TMath::Pi(), 900,-3*TMath::Pi(), 3*TMath::Pi());
  // fhSvcVal->BookHisto2List("ECalSel",Form("ECal_SC_ThetaSumVsDeltaTheta_eIoni"), 900,0 , 3*TMath::Pi(), 900,-3*TMath::Pi(), 3*TMath::Pi());
  // fhSvcVal->BookHisto2List("ECalSel",Form("ECal_SC_ThetaSumVsDeltaTheta_eBrem"), 900,0, 3*TMath::Pi(), 900,-3*TMath::Pi(), 3*TMath::Pi());
  // fhSvcVal->BookHisto2List("ECalSel",Form("ECal_SC_ThetaSumVsDeltaTheta_annihil"), 900,0 , 3*TMath::Pi(), 900,-3*TMath::Pi(), 3*TMath::Pi());
  // fhSvcVal->BookHisto2List("ECalSel",Form("ECal_SC_ThetaSumVsDeltaTheta_Bhabha"), 900,0 , 3*TMath::Pi(), 900,-3*TMath::Pi(), 3*TMath::Pi());
...
Bool_t ECalSel::EvaluateResolutions_macro(){

  TFile *fileIn = new TFile(InputHistofile.Data());
  if(!fileIn) std::cout<<"File not existing"<<std::endl;
  std::cout<<"File to analyze: "<< InputHistofile.Data()<<std::endl;
  fhDTheta = (TH2D*) fileIn->Get("ECalSel/ECal_SC_DTheta_vs_ECluMin_cut")->Clone();
  fhDPhi = (TH2D*) fileIn->Get("ECalSel/ECal_SC_DPhi_vs_ECluMin_cut")->Clone();
  fhDE_E1 = (TH2D*) fileIn->Get("ECalSel/ECal_SC_DE_vs_EClu1_cut")->Clone();
  fhDE_E2 = (TH2D*) fileIn->Get("ECalSel/ECal_SC_DE_vs_EClu2_cut")->Clone();
...
Bool_t ECalSel::EvaluateResolutions(){
    Double_t EnergyVal=0.;
    double Edown=fGeneralInfo->GetEnergyMin();
    double Eup=Edown+spacing;
    TString sliceOutname;
    ...
    TH2D *hForSigma[4];
    hForSigma[0]= (TH2D*)fhDE_E1->Clone();
    hForSigma[1]= (TH2D*)fhDE_E2->Clone();
    hForSigma[2]= (TH2D*)fhDPhi->Clone();
    hForSigma[3]= (TH2D*)fhDTheta->Clone();
    
    TString gNames[4];
    gNames[0]= "SigmaE1Graph";
    gNames[1]= "SigmaE2Graph";
    gNames[2]= "SigmaDPhiGraph";
    gNames[3]= "SigmaDThetaGraph";
    ...
    for(int i =0; i<4; i++){
      gForSigma[i] = new TGraphErrors(NSlicesE);
       hForSigma[i]->Write();

    }    


    // TGraphErrors *SigmaE1Graph = new TGraphErrors(NSlicesE);
    // TGraphErrors *SigmaE2Graph = new TGraphErrors(NSlicesE);
    // TGraphErrors *SigmaDPhiGraph = new TGraphErrors(NSlicesE);
    // TGraphErrors *SigmaDThetaGraph = new TGraphErrors(NSlicesE);
    ...
    for(int iRes=0; iRes<4; iRes++){
          for(Int_t iSlice=0; iSlice<NSlicesE; iSlice++){
                ...
                hForSigma[iRes]->GetXaxis()->SetRangeUser(Edown,Eup);
                TH1D *hForSigma_pro =(TH1D*) hForSigma[iRes]->ProjectionY();
                hForSigma_pro->SetName(Form("%s_%i", gNames[iRes].Data(), iSlice));
                hForSigma_pro->SetTitle(Form("%s_%i", gNames[iRes].Data(), iSlice));
                hForSigma_pro->Write(); 
                hForSigma_pro->SetName(Form("FithForSigma_pro_%i",iSlice));
                if(hForSigma_pro->Integral()==0) continue;
                TF1 *gausf = new TF1("gausf", "gaus", -100, 100); 
                
                TFitResultPtr GausFitResults= hForSigma_pro->Fit(gausf,"RESQ");
                ...
                gForSigma[iRes]->SetPoint(iSlice,EnergyVal, gausf->GetParameter(2));
                gForSigma[iRes]->SetPointError(iSlice, 0, gausf->GetParError(2));
                Eup+=spacing;
                Edown+=spacing;
          ...
          gForSigma[iRes]->SetTitle(Form("Fit Resolution %s; Cluster Energy [MeV]; Resolution",gNames[iRes].Data()));
          gForSigma[iRes]->SetName(gNames[iRes].Data());
          gForSigma[iRes]->Write();
...
Bool_t ECalSel::FitTagProbeEff(){
      ...
      if(expGaus->GetChisquare()/expGaus->GetNDF()<0.6 ||expGaus->GetChisquare()/expGaus->GetNDF()>1.5) std::cout<<"BIG PROBLEM: Chi2 is out of range: "<<expGaus->GetChisquare()/expGaus->GetNDF()<<std::endl;
      ProYTagAll->Write();
      TF1 *expBkg = new TF1("expBkg", "exp([0]+[1]*x)", -60,60);
      expBkg->SetParameter(0, expGaus->GetParameter(0));
      expBkg->SetParameter(1, expGaus->GetParameter(1));
      ...
      ofstream IntegratedEfficiency(Form("/data9Vd1/padme/dimeco/TagAndProbeOut/DATAout/IntegratedEfficiency_%s.txt",fNRun.Data()));
      ...
      std::cout<<" Integrated eff in --> /data9Vd1/padme/dimeco/TagAndProbeOut/DATAout/IntegratedEfficiency_"<<fNRun.Data()<<".txt "<<std::endl;
