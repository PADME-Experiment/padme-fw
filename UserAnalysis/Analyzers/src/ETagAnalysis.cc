...
// Last modified by A. Frankenthal 26/09/2022
...
Bool_t ETagAnalysis::Process(){
  ...
  if( (trigMask & (1 << 0)) ) {
    ...
    for (int iecal = 0; iecal < NClustersECAL; iecal++) {
	    TVector3 pos_ecal =  fEvent->ECalRecoCl->Element(iecal)->GetPosition();
	    x_ecal = pos_ecal.X(); // mm
	    y_ecal = pos_ecal.Y(); // mm
	    for(int icl = 0; icl < NClustersETag; icl++) {
		    // compute vertical etag bar position from ID
