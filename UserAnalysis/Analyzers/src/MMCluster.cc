...
bool MMCluster::MergeAcrossPlanes(MMCluster* inputclus){
  ...
  if (TMath::Abs(fTracos.inter - inputclu->GetTracklet().inter) > dvAtMeshMax) return kFALSE;
  ...
  if (fMMHitsInClu.at(0)->GetMMchInfo().view == XVIEW){
    for (int i=0; i<2; i++) fFitter.Config().ParSettings(1+2*i).Fix(); // fix parameters 1,3
    fitmode = 11; // fit XZ view and DZ
  } else if (fMMHitsInClu.at(0)->GetMMchInfo().view == YVIEW){
    for (int i=0; i<2; i++) fFitter.Config().ParSettings(2*i).Fix(); // fix parameters 0,2    
    fitmode = 10; // fit YZ view and DZ
  }

  fMMTrackFcn.setFitMode(fFitMode);
  vector<MMSoftHit*> hitArray;
  for (Int_t i= 0; i<fMMHitsInClu.size(); i++) hitArray.push_back(fMMHitsInClu.at(i));
  for (Int_t i= 0; i<inputclus->GetfHitsVectorSize(); i++) hitArray.push_back(inputclus->GetHit(i));
  ...
  for (Int_t i= 0; i<inputclus->GetfHitsVectorSize(); i++) {
  ...
  for (int i=0; i<5; i++) fTracos.par[i] = result.GetParams()[i];
  if (fMMHitsInClu.at(0)->GetMMchInfo().view == XVIEW){
    fTracos.slope = (fTracos.par[2]-fTracos.par[0])/(GeneralInfo::GetInstance()->GetMMPosPlaneZ(1)-GeneralInfo::GetInstance()->GetMMPosPlaneZ(0));
    fTracos.inter = fTracos.par[0] + fTracos.slope*(GeneralInfo::GetInstance()->GetMMPosPlaneZ(2)-GeneralInfo::GetInstance()->GetMMPosPlaneZ(0));
  } else {
    fTracos.slope = (fTracos.par[3]-fTracos.par[1])/(GeneralInfo::GetInstance()->GetMMPosPlaneZ(1)-GeneralInfo::GetInstance()->GetMMPosPlaneZ(0));
    fTracos.inter = fTracos.par[1] + fTracos.slope*(GeneralInfo::GetInstance()->GetMMPosPlaneZ(2)-GeneralInfo::GetInstance()->GetMMPosPlaneZ(0));
  }
