...
Bool_t MMTrackDevel::InitHistos(Int_t nRun){
  ...
  for (int quad = 0; quad<4; quad++) fHS->BookHisto2List("MMTrackDevel",Form("MM_ECAL_dY_vs_dz_Q%d_clu1",quad),100,-200,200,1000,-100,100);
  for (int quad = 0; quad<4; quad++) fHS->BookHisto2List("MMTrackDevel",Form("MM_ECAL_dX_vs_dz_Q%d_clu1",quad),100,-200,200,1000,-100,100);
  
  for (int quad = 0; quad<4; quad++) fHS->BookHisto2List("MMTrackDevel",Form("MM_ECAL_Y_vs_IPtheta_Q%d_clu1",quad),100,-0.01,0.01,1200,-600,600);
  for (int quad = 0; quad<4; quad++) fHS->BookHisto2List("MMTrackDevel",Form("MM_ECAL_X_vs_IPtheta_Q%d_clu1",quad),100,-0.01,0.01,1200,-600,600);

  for (int quad = 0; quad<4; quad++) fHS->BookHisto2List("MMTrackDevel",Form("MM_ECAL_dY_vs_Nhit_Q%d_clu0",quad),10,0,10,1000,-100,100);
  for (int quad = 0; quad<4; quad++) fHS->BookHisto2List("MMTrackDevel",Form("MM_ECAL_dX_vs_Nhit_Q%d_clu0",quad),10,0,10,1000,-100,100);
 
  for (int quad = 0; quad<4; quad++) fHS->BookHistoList("MMTrackDevel",Form("MM_ECAL_YAtTarget_Q%d_clu1",quad),400,-200.,200.);
  for (int quad = 0; quad<4; quad++) fHS->BookHistoList("MMTrackDevel",Form("MM_ECAL_XAtTarget_Q%d_clu1",quad),400,-200.,200.);
...
Bool_t MMTrackDevel::Process(){
  ...
  for (int clumode=0; clumode<2; clumode++){
    for (int ipmode=0; ipmode<ipmodeMax[clumode]; ipmode++){
      fHS->FillHistoList("MMTrackDevel",Form("MM_NclusIPMode%dCluMode%d",ipmode,clumode),fMMClusteringInstance->GetMMClusterLength(0,0),1.);
    }
  }
  ...
  for(int iclu=0; iclu<(int) fMMClusteringInstance->GetMMClusterLength(0,0); iclu++) {
    ...
    for(int iidx=0; iidx<(int) cluIndices.size(); iidx++) {
      ...
      if(view == YVIEW) {
	double dy_MMEcal = MMposAtEcal.Y() - y_Ecal;
	if(x_Ecal*signsQuadX[quad]>0 && y_Ecal*signsQuadY[quad]>0) fHS->FillHisto2List("MMTrackDevel",Form("MM_ECAL_dY_vs_Nhit_Q%d_clu0",quad),Nhit,dy_MMEcal,1.);
      }
      if(view == XVIEW) {
	double dx_MMEcal = MMposAtEcal.X() - x_Ecal;
