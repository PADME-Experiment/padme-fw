...
Double_t EEShapes::TF1Function(Double_t* dsqrts, Double_t* par) {
  Double_t gauss1 = 0.33 * par[0] * 1/(sqrt((par[2]*par[2] + par[5]*par[5]*par[1]*par[1]))*2*(TMath::Pi())) * exp(-0.5 * pow((dsqrts[0] - par[1]) / sqrt(par[2]*par[2] + par[5]*par[5]*par[1]*par[1]), 2));
  Double_t gauss2 = 0.67 * par[3] * 1/(sqrt((par[4]*par[4] + par[5]*par[5]*par[1]*par[1]))*2*(TMath::Pi())) * exp(-0.5 * pow((dsqrts[0] - par[1]) / sqrt(par[4]*par[4] + par[5]*par[5]*par[1]*par[1]), 2));
