...
void do_debug_run(global_config &config, StatisticsModel *full_model) {
  ...
  TFile *output_debug = TFile::Open("X17output_debug.root", "RECREATE");
  ...
  // config.mass_points.push_back(17.0);
  ...
  if (config.tossingType == kFixToFit) {
    ...
    full_model->gve_test->setVal(1.0e-4);
  ...
  for (Int_t iToy = 0; iToy < config.n_toys; ++iToy) {
    ...
    unique_ptr<RooDataSet> toys_sb{full_model->Generate(config, 1.0e-4, sb_model, fitted_values_sb)};
...
void do_exclusion_run(global_config &config, StatisticsModel *full_model) {
  ...
  TFile *output_exclusion = TFile::Open("PADME_output_exclusion.root", "RECREATE");
  ...
  for (size_t iM = 0; iM < config.mass_points.size(); iM++) {
    ...
    for (size_t iE = 0; iE < config.log_coupling_points.size(); iE++) {
      ...
      if (config.tossingType == kFixToFit) {
        // fit under s+b
        full_model->fitGlobal_parameters->assignFast(*params_init);
        full_model->gve_test->setVal(1.0);
        full_model->gve_test->setConstant(true);
  ...
  for (size_t iMass = 0; iMass < config.mass_points.size(); iMass++) {
    ...
    for (size_t iE = 0; iE < config.log_coupling_points.size(); iE++) {
      ...
      unique_ptr<RooDataSet> pseudo_observation{full_model->bkgModel_full->generate(
                                               {*full_model->SqrtS, *full_model->ScanPoint_categories},
      ...
      for (int iToy = 0; iToy < config.n_toys; iToy++) {
        ...
        unique_ptr<RooDataSet> toy_aux{full_model->bkgModel_full->generate(
            *full_model->Global_observables, 1)};
...
int main(int argc, char **argv) {
  ...
  for(int iC = 0; iC<config.nChannel; iC++){
    cout <<"iterations for adding channels: " << iC << endl;
