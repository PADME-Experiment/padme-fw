...
ChannelModel::ChannelModel(TString channelName, RooRealVar &_invMass,
    ...
    : fChannelName(channelName), invariantMass(&_invMass), dpMass(&_dpMass),
      ...
      bkg_yield_default(new RooRealVar(Form("bkg_yield_default_%s", fChannelName.Data()),
                         ...
                         bkg_mean, 1E-6, 10.)),
      bkg_yield_scale(new RooRealVar(Form("bkg_yield_scale_%s", fChannelName.Data()),
                         Form("bkg_yield_scale_%s", fChannelName.Data()),
                         1.0, 1E-6, 10.)),
      bkg_yield_true(new RooProduct(Form("bkgYieldTrue_%s", fChannelName.Data()),
                         Form("bkgYieldTrue_%s", fChannelName.Data()), {*bkg_yield_default, *bkg_yield_scale})),
      bkg_yield_sigma(new RooRealVar(Form("bkgYieldSigma_%s", fChannelName.Data()),
                         Form("bkgYieldSigma_%s", fChannelName.Data()),
                         sigma_bkg, 1.0 + 1E-6, 10.)),
      bkg_yield_constraint(new RooLognormal(Form("bkgYieldConstraint_%s", fChannelName.Data()),
...

{
  ...
  if (bkg_type == kLandau) {
    ...
    RooRealVar c0("c0", "c0", 1.88569e-02, 1.88369e-02, 1.88769e-02);
    RooRealVar c1("c1", "c1", 1.94453e-01, 1.94253e-01, 1.94653e-01);
    RooRealVar c2("c2", "c2", 9.18272e-01, 9.18072e-01, 9.18472e-01);
    RooRealVar c3("c3", "c3", 2.76931e-01, 2.76731e-01, 2.77131e-01);
    RooRealVar c4("c4", "c4", 1.19633e-01, 1.19433e-01, 1.19833e-01);
    RooRealVar c5("c5", "c5", 6.84498e-02, 6.84298e-02, 6.84698e-02);
    coeffs.add(c0);
    coeffs.add(c1);
    coeffs.add(c2);
    coeffs.add(c3);
