...
using namespace std; 
  ...
  Double_t Positron_InitialP      = 0.402;
  ...
  Double_t TargetCalo = 3640; //in mm 2023  
  ...
  TFile *fBhabhaSCh  = new TFile("/home/marco/analysis/SM_analysis/CalcHepFiles/AAProd/AAProduction_402.root"); 
  TFile *fBhabhaFull = new TFile("/home/marco/analysis/SM_analysis/CalcHepFiles/BhabhaFull/BhabhaFull_402_newcut.root"); 

  TTree *tBhabhaSCh  = (TTree*)fBhabhaSCh->Get("AAProduction_402"); 
  TTree *tBhabhaFull = (TTree*)fBhabhaFull->Get("tBhabhaFull_402"); 
  ...
  tBhabhaSCh->SetBranchAddress("GFinalP1_G1", &SFinalP1_electron);
  tBhabhaSCh->SetBranchAddress("GFinalP2_G1", &SFinalP2_electron);
  tBhabhaSCh->SetBranchAddress("GFinalP3_G1", &SFinalP3_electron);
  tBhabhaSCh->SetBranchAddress("GFinalP1_G2", &SFinalP1_positron);
  tBhabhaSCh->SetBranchAddress("GFinalP2_G2", &SFinalP2_positron);
  tBhabhaSCh->SetBranchAddress("GFinalP3_G3", &SFinalP3_positron);
  tBhabhaFull->SetBranchAddress("FFinalP1_electron", &FinalP1_Fe);
  tBhabhaFull->SetBranchAddress("FFinalP2_electron", &FinalP2_Fe);
  tBhabhaFull->SetBranchAddress("FFinalP3_electron", &FinalP3_Fe);
  tBhabhaFull->SetBranchAddress("FFinalP1_positron", &FinalP1_Fp);
  tBhabhaFull->SetBranchAddress("FFinalP2_positron", &FinalP2_Fp);
  tBhabhaFull->SetBranchAddress("FFinalP3_positron", &FinalP3_Fp);
  ...
  for(Long64_t k=0; k<nentries; k++){
    ...
    //TRANSVERSE FINAL POSITION - THETA PHI DEF
  ...
  Double_t D_C = 3.5;
  Double_t A_C = 12.;
  Double_t TThickness = 0.0097; //in cm
  ...
  Double_t Sigma_Full = 5.135495e11; //pb 
  Double_t Sigma_FullP5 = Sigma_Full*1.05; //pb 
  Double_t Sigma_FullM5 = Sigma_Full*0.95; //pb 
  Double_t Sigma_S    = 2.024616e9;  //pb 
  ...
  Double_t En_min = 0.100; 
  Double_t Radius_min = 90.; 
  Double_t MagnetShadow = 266.;//modified
    ...
    if(Radius_SPos[k]>Radius_min && Radius_SElec[k]>Radius_min){
      if(Radius_SPos[k]<Radius_max && Radius_SElec[k]<Radius_max){
        if(SFinalEn_positron[k]>En_min && SFinalEn_electron[k]>En_min){
          if(TMath::Abs(Pos_Sy[k]) < MagnetShadow && TMath::Abs(Elec_Sy[k]) < MagnetShadow){
            Scan_S++;
    ...
    if(Radius_Fp[k]>Radius_min && Radius_Fe[k]>Radius_min){
        if(Radius_Fp[k]<Radius_max && Radius_Fe[k]<Radius_max){
            if(FinalEn_Fp[k]>En_min && FinalEn_Fe[k]>En_min){
                if(TMath::Abs(Fp_y[k]) < MagnetShadow && TMath::Abs(Fe_y[k]) < MagnetShadow){
                    Scan_Full++;
                }
  ...
  for(int k = 0; k<1000; k++){
    Squared_gve_stat[k] = ((X20_discover_stat * 1.5 * A_C * 1e24 * 9e-8)/(PoT * PreFactor_X20 * Na * Z_C * D_C)) / TMath::Gaus(E_res, E_beam[k], 2);
    ...
    Squared_gveP5_stat[k] = ((X20_discover_P5_stat * 1.5 * A_C * 1e24 * 9e-8)/(PoT * PreFactor_X20 * Na * Z_C * D_C)) / TMath::Gaus(E_res, E_beam[k], 2);
    ...
    Squared_gveM5_stat[k] = ((X20_discover_M5_stat * 1.5 * A_C * 1e24 * 9e-8)/(PoT * PreFactor_X20 * Na * Z_C * D_C)) / TMath::Gaus(E_res, E_beam[k], 2);
    ...
    Squared_gve[k] = ((X20_discover * 1.5 * A_C * 1e24 * 9e-8)/(PoT * PreFactor_X20 * Na * Z_C * D_C)) / TMath::Gaus(E_res, E_beam[k], 2);
    if(Squared_gve[k] > 1e0) Squared_gve[k] = 1e0;
    gve[k] = TMath::Sqrt(Squared_gve[k]);
    // cout<<"E_beam: "<<E_beam[k]<<" gve: "<<gve[k]<<endl;
    Squared_gveP5[k] = ((X20_discover_P5 * 1.5 * A_C * 1e24 * 9e-8)/(PoT * PreFactor_X20 * Na * Z_C * D_C)) / TMath::Gaus(E_res, E_beam[k], 2);
    if(Squared_gveP5[k] > 1e0) Squared_gveP5[k] = 1e0;
    gveP5[k] = TMath::Sqrt(Squared_gveP5[k]);
    // cout<<"E_beam: "<<E_beam[k]<<" gveP5: "<<gveP5[k]<<endl;
    Squared_gveM5[k] = ((X20_discover_M5 * 1.5 * A_C * 1e24 * 9e-8)/(PoT * PreFactor_X20 * Na * Z_C * D_C)) / TMath::Gaus(E_res, E_beam[k], 2);
    if(Squared_gveM5[k] > 1e0) Squared_gveM5[k] = 1e0;
    gveM5[k] = TMath::Sqrt(Squared_gveM5[k]);
    // cout<<"E_beam: "<<E_beam[k]<<" gveM5: "<<gveM5[k]<<endl;

    // file<<Mass_X20[k]<<"  "<<gve[k]<<"\n";
    // file1<<Mass_X20[k]<<"  "<<gve_stat[k]<<"\n";
  ...
  Production->SetTitle("NPoT = 1e10; M_{X20} [MeV]; N_{tot}^{X20}");
  ...
  Production->SetLineWidth(1);

  TGraph *ProductionP5 = new TGraph(1000, Mass_X20, prod_X20P5);
  ProductionP5->SetTitle("NPoT = 1e10; M_{X20} [MeV]; N_{tot}^{X20}");
  ...
  ProductionP5->SetLineWidth(1);

  TGraph *ProductionM5 = new TGraph(1000, Mass_X20, prod_X20M5);
  ProductionM5->SetTitle("NPoT = 1e10; M_{X20} [MeV]; N_{tot}^{X20}");
  ...
  MGProduction->SetTitle("Vector boson production at E_{beam}=430 MeV; M_{X20} [MeV]; N_{tot}^{X20}");
