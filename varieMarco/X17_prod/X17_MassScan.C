...
#include <iostream>
  ...
  Double_t prod_X17[21][20000], prod_X171[21][20000],  Squared_gve[21][20000], Squared_gve1[21][20000];
  ...
  for(int l = 0; l<21; l++){
    for(int k = 0; k<20000; k++){
      ...
      //Squared_gve[l][k] = ((X17_discover[9] * 0.7 * A_C * 1e24 * 9e-8)/( PoT[9] * PreFactor_X17 * Na * Z_C * D_C)) / (TMath::Gaus(E_res[l], E_beam[k], 0.7));
      //Squared_gve1[l][k] = ((X17_discover[0] * 0.7 * A_C * 1e24 * 9e-8)/( PoT[0] * PreFactor_X17 * Na * Z_C * D_C)) / (TMath::Gaus(E_res[l], E_beam[k], 0.7));
  ...
  double max_value[20000], max_value1[20000], min_value[20000], min_value1[20000], Prod550[20000], Na64[20000], Brem550[20000];
  double check1[21], check2[21], check3[21], check4[21];
  for(int k = 0;k<20000; k++){
    for(int l = 0; l<21; l++){
      check1[l] = prod_X17[l][k];
      check4[l] = prod_X171[l][k];      
      //check2[l] = Squared_gve[l][k]; 
      //check3[l] = Squared_gve1[l][k]; 
    ...
    max_value[k] = TMath::MaxElement(20, check1);
    max_value1[k] = TMath::MaxElement(20, check4);
    
    //min_value[k] = TMath::MinElement(20, check2);
    //min_value1[k] = TMath::MinElement(20, check3);
    //Na64[k] = 4e-8;
  }
  

  TGraph *Production = new TGraph(20000, Mass_X17, max_value);
  Production->SetTitle("NPoT = 1e11; M_{X17} [MeV]; N_{tot}^{X17}");
  Production->SetMaximum(2e4); Production->SetMinimum(1e-2);
  Production->GetXaxis()->SetRangeUser(16.09, 17.56);
  Production->SetLineColor(kTeal-6);
  Production->SetLineWidth(2);

  TGraph *Production1 = new TGraph(20000, Mass_X17, max_value1);
  Production1->SetTitle("NPoT = 1e10; M_{X17} [MeV]; N_{tot}^{X17}");
  Production1->SetMaximum(2e4); Production->SetMinimum(1e-2);
  Production1->GetXaxis()->SetRangeUser(16.09, 17.56);
  Production1->SetLineColor(kBlue-2);
  Production1->SetLineWidth(2);
  
  /*TGraph *GgVe = new TGraph(20000, Mass_X17, min_value);
  GgVe->SetTitle("NPoT=1e11; M_{X17} [MeV]; g_{V_{e}}^{2}");
  GgVe->SetMaximum(4e-6); GgVe->SetMinimum(8e-10);
  GgVe->GetXaxis()->SetRangeUser(16.09, 17.56);
  GgVe->SetLineColor(kTeal-6);
  GgVe->SetLineWidth(2);

  TGraph *GgVe1 = new TGraph(20000, Mass_X17, min_value1);
  GgVe1->SetTitle("NPoT=1e10; M_{X17} [MeV]; g_{V_{e}}^{2}");
  GgVe1->SetMaximum(4e-6); GgVe->SetMinimum(8e-10);
  GgVe1->GetXaxis()->SetRangeUser(16.09, 17.56);
  GgVe1->SetLineColor(kBlue-2);
  GgVe1->SetLineWidth(2);*/

  /*TGraph *X17_550 = new TGraph(20000, Mass_X17, Prod550);
  X17_550->SetTitle("e^{+}e^{-}->#gamma X17, E_{e^{+}}~550 MeV; M_{X17} [MeV]; N_{tot}^{X17}");
  X17_550->SetMaximum(2e4); X17_550->SetMinimum(1e-2);
  X17_550->GetXaxis()->SetRangeUser(16.09, 17.56);
  X17_550->SetLineColor(kRed);
  X17_550->SetLineWidth(2);
  X17_550->SetLineStyle(2);

  TGraph *BREM_550 = new TGraph(20000, Mass_X17, Brem550);
  BREM_550->SetTitle("Bremm., E_{e^{+}}~550 MeV; M_{X17} [MeV]; N_{tot}^{X17}");
  BREM_550->SetMaximum(2e4); X17_550->SetMinimum(1e-2);
  BREM_550->GetXaxis()->SetRangeUser(16.09, 17.56);
  BREM_550->SetLineColor(kMagenta+1);
  BREM_550->SetLineWidth(2);
  BREM_550->SetLineStyle(2);*/

  /*TGraph *NA64limit = new TGraph(20000, Mass_X17, Na64);
  NA64limit->SetTitle("NA64 g_{V_{e}}^{2}=4e-8; M_{X17} [MeV]; g_{V_{e}}^{2}");
  NA64limit->SetMaximum(4e-6); NA64limit->SetMinimum(8e-10);
  NA64limit->GetXaxis()->SetRangeUser(16.09, 17.56);
  ...
  //MGProd->GetXaxis()->SetRangeUser(16.09, 17.56);
  ...
  //MGCoupling->SetTitle("g_{V_{e}}^{2} Mass Scan; M_{X17} [MeV]; g_{V_{e}}^{2}");
