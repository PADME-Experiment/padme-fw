...
#include <iostream>
  ...
  Double_t prod_X17[21][1000], prod_X171[21][1000],  Squared_gve[21][1000], Squared_gve1[21][1000];
  ...
  ofstream file;
  file.open("gVe_MANCINI_10e11.lmt");

  ofstream file1;
  file1.open("gVe_MANCINI_10e10.lmt");
  ...
  for(int l = 0; l<21; l++){
    for(int k = 0; k<1000; k++){
      ...
      //Squared_gve1[l][k] = ((X17_discover[0] * 0.7 * A_C * 1e24 * 9e-8)/( PoT[0] * PreFactor_X17 * Na * Z_C * D_C)) / (TMath::Gaus(E_res[l], E_beam[k], 0.7) + (TMath::Gaus(E_res[l+1], E_beam[k], 0.7)));
  ...
  double check1[20], check2[20], check3[21], check4[21];
  double check31[21], check41[21];
  ...
  for(int k = 0;k<1000; k++){
    ...
    max_value1[k] = TMath::MaxElement(20, check2);
    
    //min_value[k] = TMath::MinElement(20, check3);
    //min_value1[k] = TMath::MinElement(20, check4);

    //A[k] = TMath::Sqrt(min_value[k]);
    //B[k] = TMath::Sqrt(min_value1[k]);
    
    //file<<Mass_X17[k]<<"  "<<A[k]<<"\n";;
    //file1<<Mass_X17[k]<<"  "<<B[k]<<"\n";;
  }
  ...
  Production->SetTitle("NPoT_{peak} = 1e11; M_{X17} [MeV]; N_{tot}^{X17}");
  ...
  Production1->SetTitle("NPoT_{peak} = 1e10; M_{X17} [MeV]; N_{tot}^{X17}");
  ...
  GgVe1->SetTitle("NPoT=1e10; M_{X17} [MeV]; g_{V_{e}}");
  ...
  //BREM_550->SetMaximum(2e4); BREM_550->SetMinimum(1e-2);
