...
#include <iostream>
  ...
  Double_t NC_PADME_S1[20], NC_PADME_T1[20],NC_PADME_AA1[20], NC_PADME_X171[20][10];
  Double_t Squared_gve[20], Squared_gve1[20];
  Double_t Scan_S[21][32][28], Scan_T[21][32][28], Scan_AA[21][32][28];
  Double_t S_ACC[21][32][28], T_ACC[21][32][28], AA_ACC[21][32][28];
  Double_t Sensitivity[20][10], Sensitivity1[20][10], LowerLimitX17[20], X17_produced[20], LowerLimitX171[20], X17_produced1[20];
  ...
  //MAKE A COUNTER - GEOMETRY CUTS
   ...
   TH2F *ACC_RATIO2 = new TH2F("ACC_RATIO2", "Ratio between S- and T-Channel Acceptance as a function of R_{min} and energy", 27, 77.5, 212.5, 28, 0.0675, 0.14);
   TH2F *ACC_RATIO3 = new TH2F("ACC_RATIO3", "Ratio between S- and T-Channel Acceptance as a function of R_{max} and R_{min}", 14, 212.5, 282.5, 27, 77.5, 212.5);
  ...
  for(Int_t l = 0; l<21; l++){
	...
	if(Radius_SPos[k]<Radius_max[l] && Radius_SElec[k]<Radius_max[l]){
	  if(SFinalEn_positron[k]>En_min[12] && SFinalEn_electron[k]>En_min[12]){
	    Scan_S[l][i][12]++;};};};
	    //SCAN2D->Fill(Radius_max[l], En_min[12]);};};};
      
      if(Radius_TPos[k]>Radius_min[i] && Radius_TElec[k]>Radius_min[i]){
	if(Radius_TPos[k]<Radius_max[l] && Radius_TElec[k]<Radius_max[l]){
	  if(TFinalEn_positron[k]>En_min[12] && TFinalEn_electron[k]>En_min[12]){
	    Scan_T[l][i][12]++; };};}
	    //SCAN2D->Fill(Radius_max[l], En_min[12]);};};};

      if(Radius_G1[k]>Radius_min[i] && Radius_G2[k]>Radius_min[i]){
	  if(Radius_G1[k]<Radius_max[l] && Radius_G2[k]<Radius_max[l]){
	    if(FinalEn_G1[k]>En_min[12] && FinalEn_G2[k]>En_min[12]){
	      Scan_AA[l][i][12]++;};};};
      //}
	}

    S_ACC[l][i][12] = Scan_S[l][i][12]/numRow;
    T_ACC[l][i][12] = Scan_T[l][i][12]/numRow;
    AA_ACC[l][i][12] = Scan_AA[l][i][12]/numRow;
    Acceptance_ratio[l][i][12] = S_ACC[l][i][12] / T_ACC[l][i][12];
    }
  }
  ...
  TH2F * SENS1 = new TH2F("SENS1", "PADME X17 detected over sqrt(BG) as a function of N-PoT and g_{Ve}", 20, 0.5e10, 2.05e11, 10, 0.5e-4, 1.05e-3);
  TH1F * DPOT = new TH1F("DPOT", "PADME X17 discovery potential as a function of N-PoT", 20, 0.5e10, 2.05e11);
  TH1F * DPOT1 = new TH1F("DPOT1", "PADME X17 discovery potential as a function of N-PoT", 20, 0.5e10, 2.05e11);
  TH1F * CL90 = new TH1F("CL90", "N^{X17} needed to reach the 90% CL", 20, 0.5e10, 2.05e11);
  TH1F * CL901 = new TH1F("CL901", "N^{X17} needed to reach the 90% CL", 20, 0.5e10, 2.05e11);
  TH1F * exclusion_X17 = new TH1F("exclusion_X17", "g_{Ve} exclusion plot as a function of N-PoT", 20, 0.5e10, 2.05e11);
  TH1F * exclusion_X171 = new TH1F("exclusion_X171", "g_{Ve} exclusion plot as a function of N-PoT", 20, 0.5e10, 2.05e11);

  //NA64 epsilon limit
  TH1F * NA64_limit = new TH1F("NA64_limit", "g_{e} NA64", 20, 0.5e10, 2.05e11);
  Double_t ge_NA64[20];
  double Disc[20], Disc1[20];
  
  for(Int_t l = 0; l<20; l++){ //POT
    //file<<"\n ---------------------PoT = "<<PoT[l]<<" -------------\n";
    NC_PADME_S[l] = S_ACC[18][2][12] * N_Schannel_C[l];
    NC_PADME_T[l] = T_ACC[18][2][12] * N_Tchannel_C[l];
    NC_PADME_AA[l] = AA_ACC[18][2][12] * N_AA_C[l];
    NC_PADME_S1[l] = S_ACC[18][22][12] * N_Schannel_C[l];
    NC_PADME_T1[l] = T_ACC[18][22][12] * N_Tchannel_C[l];
    NC_PADME_AA1[l] = AA_ACC[18][22][12] * N_AA_C[l];
    ge_NA64[l] = 2e-4;

    LowerLimitX17[l]= 1.2*TMath::Sqrt(NC_PADME_S[l] + NC_PADME_T[l]); //check prefactor: is 1.2 equal to 90% confidency level?
    X17_produced[l] = LowerLimitX17[l] / S_ACC[18][2][12]; //il numero di candidati X17 entro il calorimetro -> 90CL * Accettanza S Channel
    Squared_gve[l]= TMath::Sqrt((X17_produced[l] * 0.7)/(PoT[l]*PreFactor_X17)*(9e-8) * ((A_C * 1e24)/(Na * Z_C * D_C)));
    CL90->Fill(PoT[l], LowerLimitX17[l]);
    LowerLimitX171[l]= 1.2*TMath::Sqrt(NC_PADME_S1[l] + NC_PADME_T1[l]); //check prefactor: is 1.2 equal to 90% confidency level?
    X17_produced1[l] = LowerLimitX171[l] / S_ACC[18][22][12]; //il numero di candidati X17 entro il calorimetro -> 90CL * Accettanza S Channel
    Squared_gve1[l]= TMath::Sqrt((X17_produced1[l] * 0.7)/(PoT[l]*PreFactor_X17)*(9e-8) * ((A_C * 1e24)/(Na * Z_C * D_C)));
    exclusion_X17->Fill(PoT[l], Squared_gve[l]);
    NA64_limit->Fill(PoT[l], ge_NA64[l]);
    CL901->Fill(PoT[l], LowerLimitX171[l]);
    exclusion_X171->Fill(PoT[l], Squared_gve1[l]);
    ...
    for(Int_t j = 0; j<10; j++){ //EPSILON
      ...
      NC_PADME_X17[l][j] = S_ACC[18][2][12] * N_X17_C[l][j]; // questi sono il numero di X17 prodotti dalla formula 13_proceedings moltiplicati per S_Accettanza -> QUELLI RIVELATI DAL CALO
      NC_PADME_X171[l][j] = S_ACC[18][22][12] * N_X17_C[l][j];
      ...
      Sensitivity1[l][j] = NC_PADME_X171[l][j] / TMath::Sqrt(NC_PADME_X171[l][j] + NC_PADME_S[l] + NC_PADME_T[l]); //tiene conto di tutte le accettanze
      
      SENS->Fill(PoT[l], g_ve[j], Sensitivity[l][j]);
      SENS1->Fill(PoT[l], g_ve[j], Sensitivity1[l][j]);
      if(Sensitivity[l][j]<5){Disc[l] = g_ve[j];}
      if(Sensitivity1[l][j]<5){Disc1[l] = g_ve[j];}
					//file<<"N_tot X17 : "<<NC_PADME_X17[l][j]<<"\n";
      cout<<"Sensitivity: "<<Sensitivity[l][j]<<endl;
      
    }
    DPOT->Fill(PoT[l], Disc[l]);
    DPOT1->Fill(PoT[l], Disc1[l]);
  }
  //file.close();
  ...
  CL90->SetXTitle("N-PoT"); CL90->SetYTitle("N^{X17}");
  exclusion_X17->SetXTitle("N-PoT"); exclusion_X17->SetYTitle("g_{V_{e}}");
  NA64_limit->SetXTitle("N-PoT"); NA64_limit->SetYTitle("g_{V_{e}}");
  
  exclusion_X17->SetMaximum(3e-4); exclusion_X17->SetMinimum(7e-5);
  NA64_limit->SetMaximum(3e-4); NA64_limit->SetMinimum(7e-5);
  ...
  DPOT->SetXTitle("N-PoT"); DPOT->SetYTitle("g_{V_{e}}");
  DPOT1->SetXTitle("N-PoT"); DPOT1->SetYTitle("g_{V_{e}}");
  DPOT->SetMaximum(2e-3); DPOT->SetMinimum(9e-5);
  DPOT1->SetMaximum(2e-3); DPOT1->SetMinimum(9e-5);
  ...
  TCanvas *Sens3D1 = new TCanvas("Sens3D1", "PADME X17 Sensitivity as a function of N-PoT and epsilon", 900, 700);
  Sens3D1->SetGrid();
  gStyle->SetOptStat(kFALSE);
  SENS1->Draw("colz");
  DPOT1->Draw("HIST same");

  TCanvas *CL90_1D = new TCanvas("CL90_1D", "PADME X17 90% Confidence level as a function of N-PoT", 900, 700);
  CL90_1D->SetGrid();
  gStyle->SetOptStat(kFALSE);
  CL90->Draw("HIST");
  CL901->Draw("HIST SAME");

  TCanvas *CDiscovery = new TCanvas("CDiscovery", "PADME X17 discovery potential", 900, 700);
  CDiscovery->SetGrid();
  gStyle->SetOptStat(kFALSE);
  DPOT->Draw("HIST");
  DPOT1->Draw("HIST SAME");
