...
#include <iostream>
  ...
  Double_t TargetCalo = 3720; //in mm
  ...
  TFile *f3 = new TFile("/home/marco1/Documenti/SM_analysis/BhabhaFull_1e6.root");

  TTree *t1 = (TTree*)f1->Get("T1");
  TTree *t2 = (TTree*)f2->Get("t2");
  TTree *t3 = (TTree*)f3->Get("t5");
  ...
  t3->SetBranchAddress("FullFinalP1_electron", &FinalP1_Fe);
  t3->SetBranchAddress("FullFinalP2_electron", &FinalP2_Fe);
  t3->SetBranchAddress("FullFinalP3_electron", &FinalP3_Fe);
  t3->SetBranchAddress("FullFinalP1_positron", &FinalP1_Fp);
  t3->SetBranchAddress("FullFinalP2_positron", &FinalP2_Fp);
  t3->SetBranchAddress("FullFinalP3_positron", &FinalP3_Fp);
  ...
  for(Long64_t k=0; k<nentries; k++){
    ...
    FinalEn_Fp[k] = TMath::Abs(Final_FpMomentum[k]);
    FinalEn_Fe[k] = TMath::Abs(Final_FeMomentum[k]);
  ...
  Double_t Sigma_Full = 5.11e11; //2.649369e10; pb
  Double_t Sigma_S = 3.020595e8; //pb
  Double_t Sigma_T = 5.26e11; //pb -> FOCUS IR divergence saved by energy cut on CalcHEP (E_min = 0.001 GeV)
  Double_t Sigma_AA = 2.732152e9; //pb

  Double_t Luminosity_C;
  //Double_t Luminosity_Si;
  Double_t N_Full_C[20], N_Schannel_C[20], N_Tchannel_C[20], N_X17_C[20][10];
  ...
  cout<<"###################### paper X17 values ############################"<<endl;

  Double_t En_min[23]; // fissata energia del secondo valore a 30 MeV - prima stava a 50
  ...
  Double_t Radius_SPos[1000000], Radius_SElec[1000000], Radius_TPos[1000000], Radius_TElec[1000000], Radius_Fp[1000000], Radius_Fe[1000000];
  //Double_t N_PADME_S[21][28], N_PADME_T[21][28],N_PADME_AA[21][28], N_PADME_X17[21][28];
  Double_t NC_PADME_S[20], NC_PADME_T[20], NC_PADME_Full[20], NC_PADME_X17[20][10];
  Double_t Squared_gve[20];
  Double_t Scan_S[21][23], Scan_T[21][23], Scan_Full[21][23];
  Double_t S_ACC[21][23], T_ACC[21][23], Full_ACC[21][23];
  ...
  //MAKE A COUNTER - GEOMETRY CUTS
   ...
   TH2F *h9 = new TH2F("h9", "S-Channel positron R_{max} = 270 mm", CaloRes, -300, 300, CaloRes, -300, 300);  //istogramma 2d per tutti gli eventi con soli tagli geometrici
   TH2F *h10 = new TH2F("h10", "S-Channel electron R_{max} = 270 mm", CaloRes, -300, 300, CaloRes, -300, 300);

   TH2F *h11 = new TH2F("h11", "S-Channel positron R_{max} = 270 mm", CaloRes, -300, 300, CaloRes, -300, 300);  //istogramma 2d con tagli geometrici e energetici
   TH2F *h12 = new TH2F("h12", "S-Channel electron R_{max} = 270 mm ", CaloRes, -300, 300, CaloRes, -300, 300);

   TH2F *h13 = new TH2F("h13", "T-Channel positron R_{max} = 270 mm", CaloRes, -300, 300, CaloRes, -300, 300);  //istogramma 2d per tutti gli eventi con soli tagli geometrici
   TH2F *h14 = new TH2F("h14", "T-Channel electron R_{max} = 270 mm", CaloRes, -300, 300, CaloRes, -300, 300);
   
   TH2F *h15 = new TH2F("h15", "T-Channel positron R_{max} = 270 mm", CaloRes, -300, 300, CaloRes, -300, 300);  //istogramma 2d con tagli geometrici e energetici
   TH2F *h16 = new TH2F("h16", "T-Channel electron R_{max} = 270 mm", CaloRes, -300, 300, CaloRes, -300, 300);

   TH2F *h17 = new TH2F("h17", "Bhabha full positron final position R_{max} = 270 mm", CaloRes, -300, 300, CaloRes, -300, 300);  //istogramma 2d per tutti gli eventi con soli tagli geometrici
   TH2F *h18 = new TH2F("h18", "Bhabha full final position R_{max} = 270 mm", CaloRes, -300, 300, CaloRes, -300, 300);

   TH2F *h19 = new TH2F("h19", "Bhabha full final position R_{max} = 270 mm", CaloRes, -300, 300, CaloRes, -300, 300);  //istogramma 2d con tagli geometrici e energetici
   TH2F *h20 = new TH2F("h20", "Bhabha full final position R_{max} = 270 mm", CaloRes, -300, 300, CaloRes, -300, 300);

   TH1F *ACC_S = new TH1F("ACC_S", "PADME S Channel Acceptance as a function of R_{max} and E > 100 MeV - CARBON TARGET", 16, 202.5, 282.5);
   TH1F *ACC_T = new TH1F("ACC_T", "PADME T Channel Acceptance as a function of R_{max} and E > 100 MeV - CARBON TARGET", 16, 202.5, 282.5);
   TH1F *ACC_Full = new TH1F("ACC_Full", "PADME Bhabha full Acceptance as a function of R_{max} and E > 100 MeV - CARBON TARGET", 16, 202.5, 282.5);
   TH2F *ACC_RATIO = new TH2F("ACC_RATIO", "Ratio between S- and T-Channel Acceptance as a function of R_{max} and energy", 21, 177.5, 282.5, 23, 0.0275, 0.1475);
   ...
   TH1F *HScut = new TH1F("HScut", "S-channel e^{+} R_{min} = 90 mm, R_{max} = 270 mm", 60, 0, 300);
   TH1F *HTcut = new TH1F("HTcut", "T-channel e^{+} R_{min} = 90 mm, R_{max} = 270 mm", 60, 0, 300);
   TH1F *HFullcut = new TH1F("HFullcut", "Bhabha e^{+} R_{min} = 90 mm, R_{max} = 270 mm", 60, 0, 300);
   TH1F *HS1cut = new TH1F("HS1cut", "S-channel e^{-} R_{min} = 90 mm, R_{max} = 270 mm", 60, 0, 300);
   TH1F *HT1cut = new TH1F("HT1cut", "T-channel e^{-} R_{min} = 90 mm, R_{max} = 270 mm", 60, 0, 300);
   TH1F *HFull1cut = new TH1F("HA1cut", "Bhabha e^{-} R_{min} = 90 mm, R_{max} = 270 mm", 60, 0, 300);
  ...
  for(Int_t l = 0; l<21; l++){
   ...
   for(Int_t j = 0; j<23; j++){
	...
	if(Radius_Fp[k]>Radius_min && Radius_Fe[k]>Radius_min){
	  if(Radius_Fp[k]<Radius_max[l] && Radius_Fe[k]<Radius_max[l]){h17->Fill(Fp_x[k], Fp_y[k]); h18->Fill(Fe_x[k], Fe_y[k]);
	    if(FinalEn_Fp[k]>En_min[j] && FinalEn_Fe[k]>En_min[j]){h19->Fill(Fp_x[k], Fp_y[k]); h20->Fill(Fe_x[k], Fe_y[k]);
	      Scan_Full[l][j]++;
	      HFullcut->Fill(Radius_Fp[k]); HFull1cut->Fill(Radius_Fe[k]);};};};
    ...
    //Acceptance_ratio[l][j] = S_ACC[l][j] / Full_ACC[l][j];
    ...
    //file<<"N_tot S-channel : "<<N_PADME_S[l][j]<<"\n";
    //file<<"N_tot T-channel : "<<N_PADME_T[l][j]<<"\n";
    //file<<"N_tot Gamma-Gamma : "<<N_PADME_AA[l][j]<<"\n";    
    //file<<"N_tot X17 : "<<NC_PADME_X17[l][j]<<"\n\n";
  
  }
   }
   ACC_S->Fill(Radius_max[l], S_ACC[l][0]);
   ACC_T->Fill(Radius_max[l], T_ACC[l][0]);
   ACC_Full->Fill(Radius_max[l], Full_ACC[l][0]);
  ...
  TH1F * SENSrad = new TH1F("SENSErad", "PADME Sensitivity as a function of R_{max}", 21, 177.5, 282.5);
  ...
  cout<<"SChannel Acc : "<<S_ACC[19][0]<<endl;
  cout<<"TChannel Acc : "<<T_ACC[19][0]<<endl;
  cout<<"Bhabha Full Acc : "<<Full_ACC[19][0]<<endl;

  double SensRad[21];
  
  for(Int_t l = 0; l<20; l++){ //POT
    cout<<"\n ---------------------PoT = "<<PoT[l]<<" -------------"<<endl;
    NC_PADME_Full[l] = Full_ACC[19][0] * N_Full_C[l];
    NC_PADME_S[l] = S_ACC[19][0] * N_Schannel_C[l];
    NC_PADME_T[l] = T_ACC[19][0] * N_Tchannel_C[l];
    //NC_PADME_AA[l] = AA_ACC[19][11] * N_AA_C[l];
    
    ge_NA64[l] = 2e-4;

    LowerLimitX17[l]= 1.2*TMath::Sqrt(NC_PADME_Full[l]); //check prefactor: is 1.2 equal to 90% confidency level? HO MESSO IL FONDO COMPLESSIVO
    X17_produced[l] = LowerLimitX17[l] / S_ACC[19][0]; //il numero di candidati X17 entro il calorimetro -> 90CL * Accettanza S Channel
    ...
    for(Int_t j = 0; j<10; j++){ //EPSILON
      ...
      NC_PADME_X17[l][j] = S_ACC[19][0] * N_X17_C[l][j]; // questi sono il numero di X17 prodotti dalla formula 13_proceedings moltiplicati per S_Accettanza -> QUELLI RIVELATI DAL CALO
      Sensitivity[l][j] = NC_PADME_X17[l][j] / TMath::Sqrt(NC_PADME_X17[l][j] + NC_PADME_Full[l]); //tiene conto di tutte le accettanze
  ...
  for(Int_t k=0; k<21; k++){
    SensRad[k] = (S_ACC[k][0] * N_X17_C[0][1]) / TMath::Sqrt((Full_ACC[k][0]*N_Full_C[0]) + (S_ACC[k][0] * N_X17_C[0][1])); //1e10 POT - g_Ve = 2e-4 - scan sensitivity in R_max
  ...
  TH1F *h1 = new TH1F ("h1", "T-, S-channel and Bhabha-full tri-momentum distribution", 282, -0.01, 0.283);
  TH1F *h2 = new TH1F ("h2", "Electron 3P-distribution in S-Channel", 282, -0.01, 0.283);
  TH1F *h3 = new TH1F ("h3", "Positron 3P-distribution in T-Channel", 282, -0.01, 0.283);
  TH1F *h4 = new TH1F ("h4", "Electron 3P-distribution in T-Channel", 282, -0.01, 0.283);
  TH1F *h1Full = new TH1F ("h1Full", "Positron Full 3P-distribution", 282, -0.01, 0.283);
  TH1F *h2Full = new TH1F ("h2Full", "Electron Full 3P-distribution", 282, -0.01, 0.283);

  TH1F *h5 = new TH1F ("h5", "T-, S-channel and Bhabha-full energy distribution", 282, -0.01, 0.283);
  TH1F *h6 = new TH1F ("h6", "Electron energy distribution in S-Channel", 282, -0.01, 0.283);
  TH1F *h7 = new TH1F ("h7", "Positron energy distribution in T-Channel", 282, -0.01, 0.283);
  TH1F *h8 = new TH1F ("h8", "Electron energy distribution in T-Channel", 282, -0.01, 0.283);
  TH1F *h3Full = new TH1F ("h3Full", "Positron Full energy distribution", 282, -0.01, 0.283);
  TH1F *h4Full = new TH1F ("h4Full", "Electron Full energy distribution", 282, -0.01, 0.283);
  ...
  TH1F *checkFull = new TH1F("checkFull", "Final energy Bhabha Full", 1000, 0.27, 0.29);
  ...
  TH1F *hthetaFull = new TH1F("hthetaFull", "Angular distribution Bhabha Full", 1000, -0.1, 1);
  ...
  HFullcut->SetXTitle("x [mm]"); HFullcut->SetYTitle("counts");
  HS1cut->SetXTitle("x [mm]"); HS1cut->SetYTitle("counts");
  HT1cut->SetXTitle("x [mm]"); HT1cut->SetYTitle("counts");
  HFull1cut->SetXTitle("x [mm]"); HFull1cut->SetYTitle("counts");
  ...
  h1Full->Scale((1./(h1Full->Integral()))*Sigma_AA);
  h2Full->Scale(1./(h2Full->Integral()));
  h3Full->Scale((1./(h3Full->Integral()))*Sigma_AA);
  ...
  h15->Scale(1./(h15->Integral())*Sigma_T);
  h16->Scale(1./(h16->Integral())*Sigma_T);
  h17->Scale(1./(h17->Integral()));
  h18->Scale(1./(h18->Integral()));
  h19->Scale(1./(h19->Integral())*Sigma_AA);
  h20->Scale(1./(h20->Integral())*Sigma_AA);
  ...
  HFullcut->SetLineColor(kBlue);
  HS1cut->SetLineColor(kRed);
  HT1cut->SetLineColor(kGreen+2);
  HFull1cut->SetLineColor(kBlue);
  ...
  HFullcut->SetLineStyle(1); HFullcut->SetLineWidth(2);
  HS1cut->SetLineStyle(1); HS1cut->SetLineWidth(2); 
  HT1cut->SetLineStyle(1); HT1cut->SetLineWidth(2);
  HFull1cut->SetLineStyle(1); HFull1cut->SetLineWidth(2);
  ...
  TCanvas *C3P = new TCanvas("C3P", "Final 3-Momentum Positron S Channel, T Channel and Full", 900, 700);
  ...
  TCanvas *CEn = new TCanvas("CEn", "Final Energy Positron S Channe, T Channel and Bhabha Full", 900, 700);
  gStyle->SetOptStat(kFALSE);
  CEn->SetGrid();
  h5->Draw();
  h7->Draw("same HIST");
  ...
  TCanvas *C5 = new TCanvas("C5", "Final transverse position at 3000 mm from the target", 1000, 770);
  gStyle->SetOptStat(kFALSE);
  C5->Divide(3,2);
  C5->cd(1);
  ...
  TCanvas *C6 = new TCanvas("C6", "Final transverse position at 3000 mm from the target - R_{max} = 270 mm", 1000, 770);
  gStyle->SetOptStat(kFALSE);
  C6->Divide(3,2);
  C6->cd(1);
  h13->Draw("colz");
  ...
  TCanvas *C7 = new TCanvas("C7", "Final transverse position at 3000 mm from the target - R_{max} = 270 mm", 1000, 770);
  gStyle->SetOptStat(kFALSE);
  C7->Divide(3,2);
  C7->cd(1);
  h15->Draw("colz");
  ...
  TCanvas *Acc1D_Full = new TCanvas("Acc1D_Full", "PADME Bhabha Full Acceptance as a function of R_{max} and E>30 MeV", 900, 700);
  Acc1D_Full->SetGrid();
  gStyle->SetOptStat(kFALSE);
  ACC_Full->Draw("HIST");
